...
Citizen.CreateThread(function()
    ...
    if RLCore.Functions.GetPlayerData().metadata["tracker"] then
        local trackerClothingData = {outfitData = {["accessory"] = { item = 13, texture = 0}}}
        TriggerEvent('rl-clothing:client:loadOutfit', trackerClothingData)
    else
...
AddEventHandler('police:client:sendBillingMail', function(amount)
    SetTimeout(math.random(2500, 4000), function()
        ...
        TriggerServerEvent('rl-phone:server:sendNewMail', {
            ...
            message = "Best " .. gender .. " " .. charinfo.lastname .. ",<br /><br />The Central Judicial Collection Agency (CJIB) has charged the fines you have received from the police. <br /> <strong> $ ".. amount .." </strong> has been deducted from your account. <Br / > <br /> With kind regards, <br /> Mr. I. Grab",
            button = {}
        })
    end)
end)
...
AddEventHandler('police:client:SendPoliceEmergencyAlert', function()
    ...
    TriggerServerEvent("police:server:SendPoliceEmergencyAlert", streetLabel, pos, RLCore.Functions.GetPlayerData().metadata["callsign"])
    TriggerServerEvent('rl-policealerts:server:AddPoliceAlert', {
        timeOut = 10000,
        ...
        details = {
            [1] = {
                ...
                detail = MyId .. ' | ' .. RLCore.Functions.GetPlayerData().charinfo.firstname .. ' ' .. RLCore.Functions.GetPlayerData().charinfo.lastname,
            },
            [2] = {
        ...
        callSign = RLCore.Functions.GetPlayerData().metadata["callsign"],
    }, true)
end)
...
AddEventHandler('police:client:PoliceEmergencyAlert', function(callsign, streetLabel, coords)
    if (PlayerJob.name == 'police' or PlayerJob.name == 'ambulance' or PlayerJob.name == 'doctor') and onDuty then
        ...
        BeginTextCommandSetBlipName('STRING')
        AddTextComponentString("Assist Colleague")
        EndTextCommandSetBlipName(blip)
        while transG ~= 0 do
            Wait(180 * 4)
            transG = transG - 1
...
AddEventHandler('police:client:VehicleCall', function(pos, alertTitle, streetLabel, modelPlate, modelName)
    if PlayerJob.name == 'police' and onDuty then
        TriggerEvent('rl-policealerts:client:AddPoliceAlert', {
            timeOut = 4000,
            alertTitle = alertTitle,
            coords = {
                x = pos.x,
        ...
        BeginTextCommandSetBlipName('STRING')
        AddTextComponentString("Notification: Vehicle Theft")
        EndTextCommandSetBlipName(blip)
        while transG ~= 0 do
            Wait(180 * 4)
            transG = transG - 1
...
AddEventHandler('police:client:HouseRobberyCall', function(coords, msg, gender, streetLabel)
    if PlayerJob.name == 'police' and onDuty then
        ...
        SetBlipAsShortRange(blip, false)
        BeginTextCommandSetBlipName('STRING')
        AddTextComponentString("Notification: House Burglary")
        EndTextCommandSetBlipName(blip)
        while transG ~= 0 do
            Wait(180 * 4)
            transG = transG - 1
...
AddEventHandler('police:server:SendEmergencyMessageCheck', function(MainPlayer, message, coords)
    ...
    if ((PlayerData.job.name == "police" or PlayerData.job.name == "ambulance" or PlayerData.job.name == "doctor") and onDuty) then
        TriggerEvent('chatMessage', "112 NOTICE - " .. MainPlayer.PlayerData.charinfo.firstname .. " " .. MainPlayer.PlayerData.charinfo.lastname .. " ("..MainPlayer.PlayerData.source..")", "warning", message)
        ...
        SetBlipAsShortRange(blip, false)
        BeginTextCommandSetBlipName('STRING')
        AddTextComponentString("112 Message")
        EndTextCommandSetBlipName(blip)
        while transG ~= 0 do
            Wait(180 * 4)
            transG = transG - 1
