            ...
            for _,slot in pairs(Config.OccasionSlots) do
                ...
                if dist <= 40 then
                    ...
                    if not vehiclesSpawned then
                        ...
                        RLCore.Functions.TriggerCallback('rl-occasions:server:getVehicles', function(vehicles)
                    ...
                    if dstCheck <= 2 then
                        if not IsPedInAnyVehicle(ped) then
                            if not isConfirming then
                                ...
                                DrawText3Ds(vehPos.x, vehPos.y, vehPos.z + 1.25, RLCore.Shared.Vehicles[Config.OccasionSlots[i]["model"]]["name"]..', Prijs: $'..Config.OccasionSlots[i]["price"]..',-')
                                if Config.OccasionSlots[i]["owner"] == RLCore.Functions.GetPlayerData().citizenid then
                                ...
                                if IsControlJustPressed(0, Keys["E"]) then
                                    ...
                                    RLCore.Functions.TriggerCallback('rl-occasions:server:getSellerInformation', function(info)
                                        if info ~= nil then 
...
function spawnOccasionsVehicles(vehicles)
    ...
    if vehicles ~= nil then
        for i = 1, #vehicles, 1 do
            ...
            RLCore.Functions.SetVehicleProperties(oSlot[i]["occasionid"], json.decode(oSlot[i]["mods"]))
...
AddEventHandler('rl-occasions:client:BuyFinished', function(model, plate, mods)
    ...
    RLCore.Functions.SpawnVehicle(model, function(veh)
        ...
        Citizen.Wait(500)
        RLCore.Functions.SetVehicleProperties(veh, vehmods)
