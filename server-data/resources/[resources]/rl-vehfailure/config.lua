...
-- IMPORTANT: Some of these values MUST be defined as a floating point number. ie. 10.0 instead of 10
	...
	cfg = {
		deformationMultiplier = 1.5,					-- How much should the vehicle visually deform from a collision. Range 0.0 to 10.0 Where 0.0 is no deformation and 10.0 is 10x deformation. -1 = Don't touch
		...
		damageFactorPetrolTank = 64.0,				-- Sane values are 1 to 100. Higher values means more damage to vehicle. A good starting point is 64
		...
		cascadingFailureSpeedFactor = 4.20,			-- Sane values are 1 to 100. When vehicle health drops below a certain point, cascading failure sets in, and the health drops rapidly until the vehicle dies. Higher values means faster failure. A good starting point is 8
	
		degradingFailureThreshold = 500.0,			-- Below this value, slow health degradation will set in
		cascadingFailureThreshold = 175.0,			-- Below this value, health cascading failure will set in
