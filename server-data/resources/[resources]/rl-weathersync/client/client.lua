...
RegisterNetEvent('RLCore:Client:OnPlayerLoaded', function()
    ...
    TriggerServerEvent('rl-weathersync:server:RequestCommands')
end)

RegisterNetEvent('rl-weathersync:client:EnableSync', function()
    disable = false
    TriggerServerEvent('rl-weathersync:server:RequestStateSync')
end)

RegisterNetEvent('rl-weathersync:client:DisableSync', function()
	disable = true
...
RegisterNetEvent('rl-weathersync:client:SyncWeather', function(NewWeather, newblackout)
    CurrentWeather = NewWeather
    blackout = newblackout
end)

RegisterNetEvent('rl-weathersync:client:RequestCommands', function(isAllowed)
    if isAllowed then
...
RegisterNetEvent('rl-weathersync:client:SyncTime', function(base, offset, freeze)
    freezeTime = freeze
    timeOffset = offset
    baseTime = base
end)
