...
RegisterNetEvent("qb-realestate:client:HouseList", function()
    RLCore.Functions.TriggerCallback("qb-realestate:server:GetHouses", function(result)
        if result == nil then
            RLCore.Functions.Notify("No houses set", "error", 5000)
        else
            local MenuHouseList = {
                {
                    header = "House list",
...
RegisterNetEvent('qb-realestate:client:changetier', function(data)
    ...
    if dialog then
        ...
        if tier <= Config.MaxTier and tier >= 1 then
            ...
            RLCore.Functions.Notify('Please enter number smaller then ' ..Config.MaxTier, 'error')
        end
    end
end)
