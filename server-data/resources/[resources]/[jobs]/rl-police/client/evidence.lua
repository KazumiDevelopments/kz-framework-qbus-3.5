...
AddEventHandler("evidence:client:ClearBlooddropsInArea", function()
	...
	local blooddropList = {}
	RLCore.Functions.Progressbar("clear_blooddrops", "Collecting Blood Samples", 5000, false, true, {
		...
		if Blooddrops ~= nil and next(Blooddrops) ~= nil then 
			...
			RLCore.Functions.Notify("Blood taken away")
		end
    end, function() -- Cancel
        RLCore.Functions.Notify("Blood not taken away", "error")
    end)
end)
...
AddEventHandler("evidence:client:ClearCasingsInArea", function()
	local pos = GetEntityCoords(PlayerPedId())
	local casingList = {}
	RLCore.Functions.Progressbar("clear_casings", "Removing Bullet Sleeves", 5000, false, true, {
        disableMovement = false,
        disableCarMovement = false,
		disableMouse = false,
		disableCombat = true,
		...
		if Casings ~= nil and next(Casings) ~= nil then 
			...
			RLCore.Functions.Notify("Bullet sleeves removed")
		end
    end, function() -- Cancel
        RLCore.Functions.Notify("Ball sleeves not removed", "error")
    end)
end)
			...
			if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, Casings[CurrentCasing].coords.x, Casings[CurrentCasing].coords.y, Casings[CurrentCasing].coords.z, true) < 1.5 then
				...
				if IsControlJustReleased(0, Keys["G"]) then
					...
					local info = {
						...
						ammolabel = Config.AmmoLabels[RLCore.Shared.Weapons[Casings[CurrentCasing].type]["ammotype"]],
...
function IsSilentWeapon(weapon)
	...
	if not retval then 
		RLCore.Functions.TriggerCallback('police:IsSilencedWeapon', function(result)
			retval = result
			return result
		end, weapon)
		Citizen.Wait(100)
...
function myBleeds()
    ...
    if not initialbleed and math.random(100) < 20 then
		if lightbleed or heavybleed or lightestbleed then
			RLCore.Functions.Notify("You notice blood oozing from your body the more you move.")
