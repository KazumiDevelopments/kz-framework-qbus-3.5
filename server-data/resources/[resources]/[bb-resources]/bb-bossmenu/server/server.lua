...
AddEventHandler("rl-bossmenu:server:withdrawMoney", function(amount)
    ...
    if Accounts[job] >= amount then
        ...
        TriggerClientEvent('RLCore:Notify', src, "Invaild Amount :/", "error")
        return
    end
...
AddEventHandler('rl-bossmenu:server:fireEmployee', function(data)
    ...
    if xEmployee then
        if xEmployee.Functions.SetJob("unemployed", '0') then
            TriggerEvent('bb-logs:server:createLog', 'bossmenu', 'Job Fire', "Successfully fired " .. GetPlayerName(xEmployee.PlayerData.source) .. ' (' .. xPlayer.PlayerData.job.name .. ')', src)

            TriggerClientEvent('RLCore:Notify', src, "Fired successfully!", "success")
            TriggerClientEvent('RLCore:Notify', xEmployee.PlayerData.source , "You got fired.", "success")

            Wait(500)
            local employees = {}
            RLCore.Functions.ExecuteSql(false, "SELECT * FROM `players` WHERE `job` LIKE '%".. xPlayer.PlayerData.job.name .."%'", function(players)
                if players[1] ~= nil then
                    for key, value in pairs(players) do
                        local isOnline = RLCore.Functions.GetPlayerByCitizenId(value.citizenid)
        ...
        RLCore.Functions.ExecuteSql(false, "SELECT * FROM `players` WHERE `citizenid` = '" .. data.source .. "' LIMIT 1", function(player)
            if player[1] ~= nil then
                ...
                RLCore.Functions.ExecuteSql(false, "UPDATE `players` SET `job` = '"..json.encode(job).."' WHERE `citizenid` = '".. data.source .."'")
                TriggerClientEvent('RLCore:Notify', src, "Fired successfully!", "success")
                ...
                TriggerClientEvent('RLCore:Notify', src, "Error. Could not find player.", "error")
            end
        end)
    end
end)
...
AddEventHandler('rl-bossmenu:server:giveJob', function(data)
    ...
    if xPlayer.PlayerData.job.isboss == true then
        if xTarget and xTarget.Functions.SetJob(xPlayer.PlayerData.job.name, '0') then
            TriggerClientEvent('RLCore:Notify', src, "You recruit " .. (xTarget.PlayerData.charinfo.firstname .. ' ' .. xTarget.PlayerData.charinfo.lastname) .. " to " .. xPlayer.PlayerData.job.label .. ".", "success")
            TriggerClientEvent('RLCore:Notify', xTarget.PlayerData.source , "You've been recruited to " .. xPlayer.PlayerData.job.label .. ".", "success")
...
AddEventHandler('rl-bossmenu:server:updateGrade', function(data)
    ...
    if xEmployee then
        if xEmployee.Functions.SetJob(xPlayer.PlayerData.job.name, data.grade) then
            TriggerClientEvent('RLCore:Notify', src, "Promoted successfully!", "success")
            TriggerClientEvent('RLCore:Notify', xEmployee.PlayerData.source , "You just got promoted [" .. data.grade .."].", "success")
        ...
        RLCore.Functions.ExecuteSql(false, "SELECT * FROM `players` WHERE `citizenid` = '" .. data.source .. "' LIMIT 1", function(player)
            if player[1] ~= nil then
                xEmployee = player[1]
                local job = RLCore.Shared.Jobs[xPlayer.PlayerData.job.name]
                local employeejob = json.decode(xEmployee.job)
                employeejob.grade = job.grades[data.grade]
                RLCore.Functions.ExecuteSql(false, "UPDATE `players` SET `job` = '"..json.encode(employeejob).."' WHERE `citizenid` = '".. data.source .."'")
                TriggerClientEvent('RLCore:Notify', src, "Promoted successfully!", "success")
...
AddEventHandler('rl-bossmenu:server:updateNearbys', function(data)
    ...
    for _, player in pairs(data) do
        local xTarget = RLCore.Functions.GetPlayer(player)
