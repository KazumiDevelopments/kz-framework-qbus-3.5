...
Citizen.CreateThread(function()
    while true do
        ...
        if insideDriftSchool then
            ...
            if repairDist[1] < 5 and health < 1000.0 and veh ~= 0 and rank == 'vanilla' then
                ...
                if IsControlJustPressed(1, 38) then
                    TriggerServerEvent("rl-driftschool:takemoney", repairCost(veh, health))
                    ...
                    if not isPurchaseSuccessful then
                        ...
                        local finished = exports["rl-taskbarskill"]:taskBar(4000,math.random(5,9))
                        if finished ~= 100 then
                            running = false
                        end
                        if finished == 100 then
                            local finished2 = exports["rl-taskbarskill"]:taskBar(2000,math.random(4,8))
            ...
            elseif testDriveDist[1] < 5 and rank == 'vanilla' then
                -- testdrive menu
...
RegisterNetEvent('rl-driftschool:tookmoney')
AddEventHandler('rl-driftschool:tookmoney', function(taken)
...
RegisterNUICallback('spawntestdrive', function(data, cb)
    ...
    if rank == nil or dist[1] > 5 then
        RLCore.Functions.Notify("Not a valid point.", "error")
		return
	end
    ...
    Citizen.CreateThread(function()
        ...
        if not DoesEntityExist(veh) then
            if IsModelInCdimage(model) and IsModelValid(model) then
                ...
                RLCore.Functions.Notify("Error spawning car.", "error")
                return
            end
...
Citizen.CreateThread(function()
    while true do
        Citizen.Wait(500)
        if insideDriftSchool then
            rank = RLCore.Functions.GetPlayerData().job.name
            Citizen.Wait(10000)
        end
    end
end)