...
AddEventHandler('apartments:server:CreateApartment', function(type, label)
    ...
    RLCore.Functions.ExecuteSql(false, "INSERT INTO `apartments` (`name`, `type`, `label`, `citizenid`) VALUES ('"..apartmentId.."', '"..type.."', '"..label.."', '"..Player.PlayerData.citizenid.."')")
    TriggerClientEvent('RLCore:Notify', src, "You have an apartment ("..label..")")
...
AddEventHandler('apartments:server:UpdateApartment', function(type)
    local src = source
    local Player = RLCore.Functions.GetPlayer(src)
    RLCore.Functions.ExecuteSql(false, "UPDATE `apartments` SET type='"..type.."' WHERE `citizenid` = '"..Player.PlayerData.citizenid.."'")

    TriggerClientEvent('RLCore:Notify', src, "You have changed apartments!")
    TriggerClientEvent("apartments:client:SetHomeBlip", src, type)
end)
...
function CreateApartmentId(type)
	...
	while not UniqueFound do
		...
		RLCore.Functions.ExecuteSql(true, "SELECT COUNT(*) as count FROM `apartments` WHERE `name` = '"..tostring(type .. AparmentId).."'", function(result)
...
function GetApartmentInfo(apartmentId)
    local retval = nil
    RLCore.Functions.ExecuteSql(true, "SELECT * FROM `apartments` WHERE `name` = '"..apartmentId.."' ", function(result)
...
function GetOwnedApartment(citizenid)
    RLCore.Functions.ExecuteSql(true, "SELECT * FROM `apartments` WHERE `citizenid` = '"..citizenid.."' ", function(result)
        if result[1] ~= nil then 
            return result[1]
        end
        return nil
...
RLCore.Functions.CreateCallback('apartments:GetAvailableApartments', function(source, cb, apartment)
    local apartments = {}
...
RLCore.Functions.CreateCallback('apartments:GetApartmentOffset', function(source, cb, apartmentId)
    local retval = 0
...
RLCore.Functions.CreateCallback('apartments:GetApartmentOffsetNewOffset', function(source, cb, apartment)
...
RLCore.Functions.CreateCallback('apartments:GetOwnedApartment', function(source, cb, cid)
    if cid ~= nil then
        RLCore.Functions.ExecuteSql(false, "SELECT * FROM `apartments` WHERE `citizenid` = '"..cid.."' ", function(result)
            if result[1] ~= nil then 
                return cb(result[1])
            end
            return cb(nil)
        ...
        local src = source
        local Player = RLCore.Functions.GetPlayer(src)
        RLCore.Functions.ExecuteSql(false, "SELECT * FROM `apartments` WHERE `citizenid` = '"..Player.PlayerData.citizenid.."' ", function(result)
            if result[1] ~= nil then 
                return cb(result[1])
            end
            return cb(nil)
...
RLCore.Functions.CreateCallback('apartments:IsOwner', function(source, cb, apartment)
	local src = source
    local Player = RLCore.Functions.GetPlayer(src)
    if Player ~= nil then
        RLCore.Functions.ExecuteSql(false, "SELECT * FROM `apartments` WHERE `citizenid` = '"..Player.PlayerData.citizenid.."' ", function(result)
...
RLCore.Functions.CreateCallback('apartments:GetOutfits', function(source, cb)
	local src = source
	local Player = RLCore.Functions.GetPlayer(src)

	if Player then
		RLCore.Functions.ExecuteSql(false, "SELECT * FROM `player_outfits` WHERE `citizenid` = '"..Player.PlayerData.citizenid.."'", function(result)
			if result[1] ~= nil then
				cb(result)
			else
				cb(nil)
...
RegisterServerEvent('rl-apartments:server:SetInsideMeta')
AddEventHandler('rl-apartments:server:SetInsideMeta', function(house, insideId, bool)
    local src = source
    local Player = RLCore.Functions.GetPlayer(src)
