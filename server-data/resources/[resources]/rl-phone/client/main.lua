...
RegisterNetEvent('rl-phone:client:RaceNotify')
AddEventHandler('rl-phone:client:RaceNotify', function(message)
...
RegisterNetEvent('rl-phone:client:AddRecentCall')
AddEventHandler('rl-phone:client:AddRecentCall', function(data, time, type)
    ...
    TriggerServerEvent('rl-phone:server:SetPhoneAlerts', "phone")
...
RegisterNUICallback('ClearRecentAlerts', function(data, cb)
    TriggerServerEvent('rl-phone:server:SetPhoneAlerts', "phone", 0)
...
RegisterNUICallback('SetBackground', function(data)
    ...
    TriggerServerEvent('rl-phone:server:SaveMetaData', PhoneData.MetaData)
end)
...
Citizen.CreateThread(function()
    while true do
        ...
        if isLoggedIn then
            RLCore.Functions.TriggerCallback('rl-phone:server:GetPhoneData', function(pData)   
...
RegisterNUICallback('HasPhone', function(data, cb)
    RLCore.Functions.TriggerCallback('rl-phone:server:HasPhone', function(HasPhone)
        cb(HasPhone)
    end)
end)
...
function OpenPhone()
    RLCore.Functions.TriggerCallback('rl-phone:server:HasPhone', function(HasPhone)
        if HasPhone then
            ...
            RLCore.Functions.TriggerCallback('rl-phone:server:GetGarageVehicles', function(vehicles)
...
RegisterNUICallback('RemoveMail', function(data, cb)
    ...
    TriggerServerEvent('rl-phone:server:RemoveMail', MailId)
    cb('ok')
end)

RegisterNetEvent('rl-phone:client:UpdateMails')
AddEventHandler('rl-phone:client:UpdateMails', function(NewMails)
    SendNUIMessage({
        action = "UpdateMails",
        Mails = NewMails
    })
...
RegisterNUICallback('AcceptMailButton', function(data)
    ...
    TriggerServerEvent('rl-phone:server:ClearButtonData', data.mailId)
end)

CallDriverTimer = GetGameTimer()-15000
RegisterNetEvent('rl-phone:client:AddTaxiCall')
AddEventHandler('rl-phone:client:AddTaxiCall', function(id, data)
...
RegisterNetEvent('rl-phone:client:RemoveTaxiCall')
AddEventHandler('rl-phone:client:RemoveTaxiCall', function(id)
    if PlayerJob.name ~= 'taxi' then
        return
    end
...
RegisterNetEvent('rl-phone:client:AcceptDriverCall')
AddEventHandler('rl-phone:client:AcceptDriverCall', function(data)
    if PlayerJob.name ~= 'taxi' then
        return
    end
...
RegisterNUICallback('AcceptTaxiCall', function(data, cb)
    ...
    TriggerServerEvent("rl-phone:server:AcceptDriverCall", tonumber(data.id))
end)
...
RegisterNUICallback('CallDriver', function(data, cb)
    ...
    TriggerServerEvent('rl-phone:server:CallDriver', { x = coords, y = coords.y, z = coords.z })
...
RegisterNUICallback('AddNewContact', function(data, cb)
    TriggerEvent("debug", 'Phone: New Contact', 'success')
    ...
    TriggerServerEvent('rl-phone:server:AddNewContact', data.ContactName, data.ContactNumber, data.ContactIban)
end)
...
RegisterNUICallback('GetProfilePicture', function(data, cb)
    ...
    RLCore.Functions.TriggerCallback('rl-phone:server:GetPicture', function(picture)
        cb(picture)
    end, number)
end)
...
RegisterNUICallback('SendMessage', function(data, cb)
    ...
    if PhoneData.Chats[NumberKey] ~= nil then
        ...
        if PhoneData.Chats[NumberKey].messages[ChatKey] ~= nil then
            ...
            TriggerServerEvent('rl-phone:server:UpdateMessages', PhoneData.Chats[NumberKey].messages, ChatNumber, false)
            ...
            TriggerServerEvent('rl-phone:server:UpdateMessages', PhoneData.Chats[NumberKey].messages, ChatNumber, true)
            NumberKey = GetKeyByNumber(ChatNumber)
            ReorganizeChats(NumberKey)
        end
    else
    ...
    RLCore.Functions.TriggerCallback('rl-phone:server:GetContactPicture', function(Chat)
        SendNUIMessage({
            action = "UpdateChat",
            chatData = Chat,
...
RegisterNetEvent('rl-phone:client:UpdateMessages')
AddEventHandler('rl-phone:client:UpdateMessages', function(ChatMessages, SenderNumber, New)
    ...
    if New then
        ...
        if PhoneData.isOpen then
            ...
            Wait(100)
            RLCore.Functions.TriggerCallback('rl-phone:server:GetContactPictures', function(Chats)
                SendNUIMessage({
                    action = "UpdateChat",
            ...
            TriggerServerEvent('rl-phone:server:SetPhoneAlerts', "whatsapp")
        end
    else
...
RegisterNetEvent("rl-phone-new:client:BankNotify")
AddEventHandler("rl-phone-new:client:BankNotify", function(text)
    TriggerEvent("debug", 'Phone: Bank Notify', 'success')
    SendNUIMessage({
        action = "Notification",
        NotifyData = {
            title = "Bank", 
...
RegisterNetEvent('rl-phone:client:NewMailNotify')
AddEventHandler('rl-phone:client:NewMailNotify', function(MailData)
    TriggerEvent("debug", 'Phone: New Mail', 'success')
    if PhoneData.isOpen then
        SendNUIMessage({
            action = "PhoneNotification",
            PhoneNotify = {
    ...
    TriggerServerEvent('rl-phone:server:SetPhoneAlerts', "mail")
end)

RegisterNUICallback('PostAdvert', function(data)
    TriggerEvent("debug", 'Phone: Post New Advert', 'success')
    TriggerServerEvent('rl-phone:server:AddAdvert', data.message)
end)

RegisterNetEvent('rl-phone:client:UpdateAdverts')
AddEventHandler('rl-phone:client:UpdateAdverts', function(Adverts, LastAd)
    TriggerEvent("debug", 'Phone: Update Adverts', 'success')
...
RegisterNUICallback('ClearAlerts', function(data, cb)
    TriggerEvent("debug", 'Phone: Clear Alerts', 'success')
    ...
    if PhoneData.Chats[ChatKey].Unread ~= nil then
        ...
        TriggerServerEvent('rl-phone:server:SetPhoneAlerts', "whatsapp", newAlerts)
...
RegisterNUICallback('PayInvoice', function(data, cb)
    TriggerEvent("debug", 'Phone: Pay Invoice', 'success')

    local society = data.society
    local amount = data.amount
    local invoiceId = data.invoiceId

    RLCore.Functions.TriggerCallback('rl-phone:server:PayInvoice', function(CanPay, Invoices)
...
RegisterNUICallback('DeclineInvoice', function(data, cb)
    TriggerEvent("debug", 'Phone: Decline Invoice', 'error')

    local sender = data.sender
    local amount = data.amount
    local invoiceId = data.invoiceId

    RLCore.Functions.TriggerCallback('rl-phone:server:DeclineInvoice', function(CanPay, Invoices)
...
RegisterNUICallback('EditContact', function(data, cb)
    TriggerEvent("debug", 'Phone: Edit Contact (' .. data.CurrentContactNumber .. ')', 'success')
    ...
    Citizen.Wait(100)
    cb(PhoneData.Contacts)
    TriggerServerEvent('rl-phone:server:EditContact', NewName, NewNumber, NewIban, OldName, OldNumber, OldIban)
end)
...
RegisterNetEvent('rl-phone:client:UpdateHashtags')
AddEventHandler('rl-phone:client:UpdateHashtags', function(Handle, msgData)
    TriggerEvent("debug", 'Phone: Update Hashtags', 'success')
...
RegisterNUICallback('GetTweets', function(data, cb)
    TriggerEvent("debug", 'Phone: Update Tweets', 'success')
    cb(PhoneData.Tweets)
end)

RegisterNUICallback('UpdateProfilePicture', function(data)
    TriggerEvent("debug", 'Phone: Update Profile Picture', 'success')
...
RegisterNUICallback('PostNewTweet', function(data, cb)
    TriggerEvent("debug", 'Phone: Post New Tweet', 'success')
    ...
    if data.Anonymous then
        ...
        TweetMessage.picture = 'https://thumbor.forbes.com/thumbor/fit-in/1200x0/filters%3Aformat%28jpg%29/https%3A%2F%2Fspecials-images.forbesimg.com%2Fimageserve%2F5efc6a13531e1500073c6521%2F0x0.jpg'
    end
    ...
    for i = 2, #Hashtag, 1 do
        ...
        if Handle ~= nil or Handle ~= "" then
            ...
            TriggerServerEvent('rl-phone:server:UpdateHashtags', Handle, TweetMessage)
        end
    end
        ...
        if Handle ~= nil or Handle ~= "" then
            ...
            if (Firstname ~= nil and Firstname ~= "") and (Lastname ~= nil and Lastname ~= "") then
                if Firstname ~= PhoneData.PlayerData.charinfo.firstname and Lastname ~= PhoneData.PlayerData.charinfo.lastname then
                    TriggerServerEvent('rl-phone:server:MentionedPlayer', Firstname, Lastname, TweetMessage)
                else
    ...
    TriggerServerEvent('rl-phone:server:UpdateTweets', PhoneData.Tweets, TweetMessage)
end)

RegisterNetEvent('rl-phone:client:TransferMoney')
AddEventHandler('rl-phone:client:TransferMoney', function(amount, newmoney)
    TriggerEvent("debug", 'Phone: Bank Transfer Notification', 'success')
    TriggerEvent("debug", 'Phone: Update Bank', 'success')
...
RegisterNetEvent('rl-phone:client:UpdateTweets')
AddEventHandler('rl-phone:client:UpdateTweets', function(src, Tweets, NewTweetData)
...
RegisterNetEvent('rl-phone:client:GetMentioned')
AddEventHandler('rl-phone:client:GetMentioned', function(TweetMessage, AppAlerts)
    ...
    if not PhoneData.isOpen then
        ...
        SendNUIMessage({ action = "PhoneNotification", PhoneNotify = { title = "You are mentioned in a Tweet above!", text = TweetMessage.message, icon = "fab fa-twitter", color = "#1DA1F2", }, })
    end
...
RegisterNUICallback('ClearMentions', function()
    ...
    TriggerServerEvent('rl-phone:server:SetPhoneAlerts', "twitter", 0)
    SendNUIMessage({ action = "RefreshAppAlerts", AppData = Config.PhoneApplications })
end)
...
RegisterNUICallback('ClearGeneralAlerts', function(data)
    SetTimeout(400, function()
        ...
        TriggerServerEvent('rl-phone:server:SetPhoneAlerts', data.app, 0)
        SendNUIMessage({ action = "RefreshAppAlerts", AppData = Config.PhoneApplications })
    end)
end)
...
RegisterNUICallback('TransferMoney', function(data, cb)
    ...
    if tonumber(PhoneData.PlayerData.money.bank) >= data.amount then
        ...
        TriggerServerEvent('rl-phone:server:TransferMoney', data.iban, data.amount)
        local cbdata = {
            CanTransfer = true,
...
RegisterNUICallback('CanTransferMoney', function(data, cb)
    ...
    if (PlayerData.money.bank - amount) >= 0 then
        RLCore.Functions.TriggerCallback('rl-phone:server:CanTransferMoney', function(Transferd)
            if Transferd then
...
RegisterNUICallback('CallContact', function(data, cb)
    RLCore.Functions.TriggerCallback('rl-phone:server:GetCallState', function(CanCall, IsOnline)
        local status = { 
            CanCall = CanCall, 
            IsOnline = IsOnline,
...
CallContact = function(CallData, AnonymousCall)
    ...
    TriggerServerEvent('rl-phone:server:CallContact', PhoneData.CallData.TargetData, PhoneData.CallData.CallId, AnonymousCall)
    TriggerServerEvent('rl-phone:server:SetCallState', true)
...
CancelCall = function()
    TriggerServerEvent('rl-phone:server:CancelCall', PhoneData.CallData)
    ...
    TriggerServerEvent('rl-phone:server:SetCallState', false)
...
RegisterNetEvent('rl-phone:client:CancelCall')
AddEventHandler('rl-phone:client:CancelCall', function()
    TriggerEvent("debug", 'Phone: Cancel Call', 'success')
...
RegisterNetEvent('rl-phone:client:GetCalled')
AddEventHandler('rl-phone:client:GetCalled', function(CallerNumber, CallId, AnonymousCall)
    TriggerEvent("debug", 'Phone: Get Called', 'success')
        ...
        if not PhoneData.CallData.AnsweredCall then
            if RepeatCount + 1 ~= Config.CallRepeats + 1 then
                if PhoneData.CallData.InCall then
                    ...
                    TriggerServerEvent('rl-phone:server:AddRecentCall', "missed", CallData)
                    break
                end
                Citizen.Wait(Config.RepeatTimeout)
            else
...
RegisterCommand('h', function()
    SendNUIMessage({ action = "RemoveOngoingMessage"})
    CancelCall()
end)
...
function AnswerCall()
    if (PhoneData.CallData.CallType == "incoming" or PhoneData.CallData.CallType == "outgoing") and PhoneData.CallData.InCall and not PhoneData.CallData.AnsweredCall then
        ...
        TriggerServerEvent('rl-phone:server:AnswerCall', PhoneData.CallData)
...
RegisterNetEvent('rl-phone:client:AnswerCall')
AddEventHandler('rl-phone:client:AnswerCall', function()
    TriggerEvent("debug", 'Phone: Answer Call', 'success')
...
RegisterNUICallback('FetchSearchResults', function(data, cb)
    RLCore.Functions.TriggerCallback('rl-phone:server:FetchResult', function(result)
        cb(result)
    end, data.input)
end)

RegisterNUICallback('SetupSellix', function(data, cb)
    RLCore.Functions.TriggerCallback('bb-pawnshop:server:getSellixList', function(boolz, dataz)
        if boolz == false then
            cb('banned')
        else
            cb(dataz)
...
RegisterNUICallback('ActivateBuyer', function(data, cb)
    if exports['bb-pawnshop']:hasActiveDelivery() == true then
        cb(1)
        return
    end

    RLCore.Functions.TriggerCallback('bb-pawnshop:server:takeOffer', function(boolz, dataz)
        if boolz == true then
...
RegisterNUICallback('FetchVehicleResults', function(data, cb)
    RLCore.Functions.TriggerCallback('rl-phone:server:GetVehicleSearchResults', function(result)
        if result ~= nil then 
...
RegisterNUICallback('FetchVehicleScan', function(data, cb)
    ...
    RLCore.Functions.TriggerCallback('rl-phone:server:ScanPlate', function(result)
        RLCore.Functions.TriggerCallback('police:IsPlateFlagged', function(flagged)
            result.isFlagged = flagged
            local vehicleInfo = RLCore.Shared.Vehicles[RLCore.Shared.VehicleModels[model]["model"]] ~= nil and RLCore.Shared.Vehicles[RLCore.Shared.VehicleModels[model]["model"]] or {["brand"] = "Onbekend merk..", ["name"] = ""}
...
RegisterNetEvent('rl-phone:client:addPoliceAlert')
AddEventHandler('rl-phone:client:addPoliceAlert', function(alertData)
    PlayerJob = RLCore.Functions.GetPlayerData().job
    if PlayerJob.name == 'police' and PlayerJob.onduty then
        TriggerEvent("debug", 'Phone: New Police Alert', 'success')
...
RegisterNUICallback('SetAlertWaypoint', function(data)
    TriggerEvent("debug", 'Phone: Set Alert Waypoint', 'success')

    local coords = data.alert.coords

    RLCore.Functions.Notify('GPS Location set: '..data.alert.title)
...
RegisterNUICallback('RemoveSuggestion', function(data, cb)
    ...
    TriggerEvent("debug", 'Phone: Remove Contact Suggestion (' .. data.number .. ')', 'success')
...
AddEventHandler('rl-phone:client:GiveContactDetails', function()
    ...
    if player ~= -1 and distance < 2.5 then
        ...
        TriggerServerEvent('rl-phone:server:GiveContactDetails', PlayerId)
...
RegisterNUICallback('DeleteContact', function(data, cb)
    TriggerEvent("debug", 'Phone: Delete Contact (' .. (data.CurrentContactName .. ',' .. data.CurrentContactNumber .. ',' .. data.CurrentContactIban) .. ')', 'success')
        ...
        if v.name == Name and v.number == Number then
            ...
            if PhoneData.isOpen then
                ...
                SendNUIMessage({
                    ...
                    NotifyData = {
                        title = "Phone", 
                        content = "You have removed contact!", 
                        icon = "fa fa-phone-alt", 
                        timeout = 3500, 
                        color = "#04b543",
                    },
    ...
    TriggerServerEvent('rl-phone:server:RemoveContact', Name, Number)
end)

RegisterNetEvent('rl-phone:client:AddNewSuggestion')
AddEventHandler('rl-phone:client:AddNewSuggestion', function(SuggestionData)
    TriggerEvent("debug", 'Phone: New Suggested Contact', 'success')
    ...
    Config.PhoneApplications["phone"].Alerts = Config.PhoneApplications["phone"].Alerts + 1
    TriggerServerEvent('rl-phone:server:SetPhoneAlerts', "phone", Config.PhoneApplications["phone"].Alerts)
end)

RegisterNUICallback('GetCryptoData', function(data, cb)
    TriggerEvent("debug", 'Phone: Get Crypto Data', 'success')

    RLCore.Functions.TriggerCallback('rl-crypto:server:GetCryptoData', function(CryptoData)
        cb(CryptoData)
    end, data.crypto)
end)

RegisterNUICallback('BuyCrypto', function(data, cb)
    TriggerEvent("debug", 'Phone: Buy Crypto', 'success')

    RLCore.Functions.TriggerCallback('rl-crypto:server:BuyCrypto', function(CryptoData)
        cb(CryptoData)
    end, data)
end)

RegisterNUICallback('SellCrypto', function(data, cb)
    TriggerEvent("debug", 'Phone: Sell Crypto', 'success')

    RLCore.Functions.TriggerCallback('rl-crypto:server:SellCrypto', function(CryptoData)
        cb(CryptoData)
    end, data)
end)

RegisterNUICallback('TransferCrypto', function(data, cb)
    TriggerEvent("debug", 'Phone: Transfer Crypto', 'success')

    RLCore.Functions.TriggerCallback('rl-crypto:server:TransferCrypto', function(CryptoData)
        cb(CryptoData)
    end, data)
end)

RegisterNetEvent('rl-phone:client:RemoveBankMoney')
AddEventHandler('rl-phone:client:RemoveBankMoney', function(amount)
...
RegisterNetEvent('rl-phone:client:AddTransaction')
AddEventHandler('rl-phone:client:AddTransaction', function(SenderData, TransactionData, Message, Title)
    TriggerEvent("debug", 'Phone: Add Transaction (' .. ( Title .. ' | ' .. Message ) .. ')', 'success')
    ...
    TriggerServerEvent('rl-phone:server:AddTransaction', Data)
end)

RegisterNUICallback('GetCryptoTransactions', function(data, cb)
    TriggerEvent("debug", 'Phone: Get Crypto Transactions', 'success')
    local Data = {
        CryptoTransactions = PhoneData.CryptoTransactions
    }
    cb(Data)
...
RegisterNUICallback('JoinRace', function(data)
    TriggerEvent("debug", 'Phone: Join Race', 'success')
    TriggerServerEvent('rl-lapraces:server:JoinRace', data.RaceData)
end)

RegisterNUICallback('LeaveRace', function(data)
    TriggerEvent("debug", 'Phone: Leave Race', 'success')
    TriggerServerEvent('rl-lapraces:server:LeaveRace', data.RaceData)
end)

RegisterNUICallback('StartRace', function(data)
    TriggerEvent("debug", 'Phone: Start Race ID ' .. data.RaceData.RaceId, 'success')
    TriggerServerEvent('rl-lapraces:server:StartRace', data.RaceData.RaceId)
end)

RegisterNetEvent('rl-phone:client:UpdateLapraces')
AddEventHandler('rl-phone:client:UpdateLapraces', function()
    TriggerEvent("debug", 'Phone: Update Races', 'success')
...
RegisterNUICallback('GetRaces', function(data, cb)
    TriggerEvent("debug", 'Phone: Get Races', 'success')
...
RegisterNUICallback('GetTrackData', function(data, cb)
    TriggerEvent("debug", 'Phone: Get Track Data', 'success')
...
RegisterNUICallback('SetupRace', function(data, cb)
    TriggerEvent("debug", 'Phone: Setup Race', 'success')
...
RegisterNUICallback('HasCreatedRace', function(data, cb)
    TriggerEvent("debug", 'Phone: Has Created Race', 'success')
...
RegisterNUICallback('StartTrackEditor', function(data, cb)
    TriggerEvent("debug", 'Phone: Start Track Editor', 'success')
...
RegisterNUICallback('GetRacingLeaderboards', function(data, cb)
    TriggerEvent("debug", 'Phone: Get Racing Leaderboards', 'success')
...
RegisterNUICallback('RaceDistanceCheck', function(data, cb)
    ...
    RLCore.Functions.TriggerCallback('rl-lapraces:server:GetRacingData', function(RaceData)
        ...
        if dist <= 115.0 then
            ...
            RLCore.Functions.Notify('You are too far from the race. Your navigation is set to the race.', 'error', 5000)
...
RegisterNUICallback('GetPlayerHouses', function(data, cb)
    ...
    RLCore.Functions.TriggerCallback('rl-phone:server:GetPlayerHouses', function(Houses)
...
RegisterNUICallback('SetHouseLocation', function(data, cb)
    ...
    RLCore.Functions.Notify("Your GPS is set to " .. data.HouseData.HouseData.adress .. "!", "success")
end)

RegisterNUICallback('RemoveKeyholder', function(data)
    TriggerEvent("debug", 'Phone: Remove Key Holder', 'success')
    TriggerServerEvent('rl-houses:server:removeHouseKey', data.HouseData.name, {
...
RegisterNUICallback('TransferCid', function(data, cb)
    ...
    RLCore.Functions.TriggerCallback('rl-phone:server:TransferCid', function(CanTransfer)
        cb(CanTransfer)
...
RegisterNUICallback('FetchPlayerHouses', function(data, cb)
    ...
    RLCore.Functions.TriggerCallback('rl-phone:server:MeosGetPlayerHouses', function(result)
        cb(result)
    end, data.input)
end)
...
RegisterNUICallback('GetCurrentLawyers', function(data, cb)
    ...
    RLCore.Functions.TriggerCallback('rl-phone:server:GetCurrentLawyers', function(lawyers)
        cb(lawyers)
    end)
end)
...
RegisterNUICallback('GetCurrentDrivers', function(data, cb)
    ...
    RLCore.Functions.TriggerCallback('rl-phone:server:GetCurrentDrivers', function(drivers, isTaxi)
...
RegisterNUICallback('InstallApplication', function(data, cb)
    ...
    if NewSlot <= Config.MaxSlots then
        TriggerServerEvent('rl-phone:server:InstallApplication', {
            app = data.app,
        })
        cb({
            app = data.app,
...
RegisterNUICallback('RemoveApplication', function(data, cb)
    TriggerEvent("debug", 'Phone: Remove Application', 'error')

    TriggerServerEvent('rl-phone:server:RemoveInstallation', data.app)
end)

RegisterNetEvent('rl-phone:RefreshPhone')
AddEventHandler('rl-phone:RefreshPhone', function()
...
RegisterNUICallback('GetTruckerData', function(data, cb)
    TriggerEvent("debug", 'Phone: Get Trucker Data', 'success')

    local TruckerMeta = RLCore.Functions.GetPlayerData().metadata["jobrep"]["trucker"]
    local TierData = exports['rl-trucker']:GetTier(TruckerMeta)
    cb(TierData)
end)
