...
addEventListener("message", function(event){
    ...
    if(item.open == true) {
        ...
        $.post('http://bb-bossmenu/closeNUI');
    }
});
...
$(document).on('click', '.fireemployee', function(e) {
    ...
    $.post('http://bb-bossmenu/fireEmployee', JSON.stringify({
        source: playerid,
    }));
});
...
$(document).on('click', '.grade-box', function(e) {
    ...
    $.post('http://bb-bossmenu/changeGrade', JSON.stringify({
        grade: gradeid,
...
$(document).on('click', '#option-recruit', function(e) {
    ...
    $.post('http://bb-bossmenu/openRecruit', JSON.stringify({}));
});

$(document).on('click', '#option-bosstash', function(e) {

    $.post('http://bb-bossmenu/openStash', JSON.stringify({}));
});
...
$(document).on('click', '.givejob', function(e) {
    ...
    $.post('http://bb-bossmenu/giveJob', JSON.stringify({
        source: playerid,
    }));
});
...
$(document).on('click', '.deposit', function(e) {
    $.post('http://bb-bossmenu/deposit', JSON.stringify({
        amount: currentDepositAmount,
    }));
});

$(document).on('click', '.withdraw', function(e) {
    $.post('http://bb-bossmenu/withdraw', JSON.stringify({
