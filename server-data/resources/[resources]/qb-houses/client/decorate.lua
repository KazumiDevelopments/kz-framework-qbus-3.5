...
local function CheckMovementInput()
	...
	if IsControlJustReleased(0, 21) then -- Left Shift
		...
		RLCore.Functions.Notify(Lang:t("info.speed").. tostring(speeds[curSpeed]))
	end
...
RegisterNetEvent('qb-houses:client:decorate', function()
	...
	if IsInside then
		if HasHouseKey then
			...
			RLCore.Functions.Notify(Lang:t("error.no_keys"), "error")
		end
	else
...
RegisterNUICallback("buySelectedObject", function(data, cb)
    RLCore.Functions.TriggerCallback('qb-houses:server:buyFurniture', function(isSuccess)
        if isSuccess then
            SetNuiFocus(false, false)
            cursorEnabled = false
            SaveDecorations()
	...
	while true do
		...
		if DecoMode then
			...
			if dist > 50.0 then
				DisableEditMode()
				closeDecorateUI()
				RLCore.Functions.Notify(Lang:t("error.out_range"), 'error')
			end
		end
	end
end)
