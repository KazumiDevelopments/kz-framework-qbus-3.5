RegisterServerEvent('rl-banking:server:Transfer')
AddEventHandler('rl-banking:server:Transfer', function(target, account, amount, note, fSteamID)
    local src = source
    ...
    local targetPly = RLCore.Functions.GetPlayer(target)
    ...
    if (account == "business") then
        ...
        if data then
            ...
            TriggerEvent('rl-banking:society:server:WithdrawMoney', src, amount, society)
            Wait(50)
