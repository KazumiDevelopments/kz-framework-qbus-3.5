...
AddEventHandler('rl-weed:client:getHousePlants', function(house)    
    RLCore.Functions.TriggerCallback('rl-weed:server:getBuildingPlants', function(plants)
        currentHouse = house
                    ...
                    if plyDistance < 0.8 then
                        ...
                        if plantData["plantStats"]["health"] > 0 then
                            if plantData["plantStage"] ~= plantData["plantStats"]["highestStage"] then
                                QBWeed.DrawText3Ds(plantData["plantCoords"]["x"], plantData["plantCoords"]["y"], plantData["plantCoords"]["z"], 'Kind: '..plantData["plantSort"]["label"]..'~w~ ['..plantData["plantStats"]["gender"]..'] | Nutrition: ~b~'..plantData["plantStats"]["food"]..'% ~w~ | Health: ~b~'..plantData["plantStats"]["health"]..'%')
                            else
                                QBWeed.DrawText3Ds(plantData["plantCoords"]["x"], plantData["plantCoords"]["y"], plantData["plantCoords"]["z"] + 0.2, 'The plant is fully grown ~g~E~w~ to harvest..')
                                QBWeed.DrawText3Ds(plantData["plantCoords"]["x"], plantData["plantCoords"]["y"], plantData["plantCoords"]["z"], 'Kind: ~g~'..plantData["plantSort"]["label"]..'~w~ ['..plantData["plantStats"]["gender"]..'] | Nutrition: ~b~'..plantData["plantStats"]["food"]..'% ~w~ | Health: ~b~'..plantData["plantStats"]["health"]..'%')
                                if IsControlJustPressed(0, QBWeed.Keys["E"]) then
                                    RLCore.Functions.Progressbar("remove_weed_plant", "Harvesting Plant", 8000, false, true, {
                        ...
                        elseif plantData["plantStats"]["health"] == 0 then
                            QBWeed.DrawText3Ds(plantData["plantCoords"]["x"], plantData["plantCoords"]["y"], plantData["plantCoords"]["z"], 'The plant is dead, ~r~E~w~ - To remove plant..')
                            if IsControlJustPressed(0, QBWeed.Keys["E"]) then
                                RLCore.Functions.Progressbar("remove_weed_plant", "Removing Plant", 8000, false, true, {
                                    disableMovement = true,
                                    disableCarMovement = true,
                                    disableMouse = false,
                                    disableCombat = true,
...
AddEventHandler('rl-weed:client:placePlant', function(type, item)
    ...
    if currentHouse ~= nil then
        if ClosestPlant == 0 then
            RLCore.Functions.Progressbar("plant_weed_plant", "Planting..", 8000, false, true, {
                disableMovement = true,
                disableCarMovement = true,
                disableMouse = false,
                disableCombat = true,
                ...
                TriggerEvent("debug", 'Weed: Place Plant', 'success')
            end, function() -- Cancel
                ClearPedTasks(ped)
                RLCore.Functions.Notify("Process canceled..", "error")
            ...
            RLCore.Functions.Notify('Not a safe place..', 'error', 3500)
        end
    else
        RLCore.Functions.Notify('It\'s not safe here..', 'error', 3500)
    end
end)
        ...
        if ClosestTarget ~= 0 then
            ...
            if plyDistance < 1.0 then
                if plantData["plantStats"]["food"] == 100 then
                    RLCore.Functions.Notify('The plant does not need nutrition..', 'error', 3500)
                else
                    RLCore.Functions.Progressbar("plant_weed_plant", "Feeding Plant", math.random(4000, 8000), false, true, {
                        disableMovement = true,
                        disableCarMovement = true,
                        disableMouse = false,
                        disableCombat = true,
                ...
                RLCore.Functions.Notify("Not the place to plant..", "error")
            end
        else
            RLCore.Functions.Notify("Not the place to plant..", "error")
        end
