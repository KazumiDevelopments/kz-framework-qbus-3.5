RegisterServerEvent('rl-banking:server:Withdraw')
AddEventHandler('rl-banking:server:Withdraw', function(account, amount, note, fSteamID)
    local src = source
    ...
    if account == "personal" then
        if amount > Player.PlayerData.money["bank"] then
            TriggerClientEvent("rl-banking:client:Notify", src, "error", "Your bank doesn't have this much money!") 
            return
        end
    ...
    if(account == "business") then
        ...
        if data then
            ...
            if sM >= amount then
                TriggerEvent('rl-banking:society:server:WithdrawMoney',src, amount, data.name)
                ...
                TriggerClientEvent("rl-banking:client:Notify", src, "error", "Not enough money current balance: €"..sM) 
            end
        end
    end
