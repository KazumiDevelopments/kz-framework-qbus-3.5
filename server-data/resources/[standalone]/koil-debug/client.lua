...
RegisterNetEvent("koil-debug:toggle")
AddEventHandler("koil-debug:toggle",function()
    ...
    while true do 
        ...
        if dickheaddebug then
            ...
            if IsControlJustReleased(0, 38) then
                if inFreeze then
                    inFreeze = false
                    RLCore.Functions.Notify('Freeze Disabled', 'error')     
                else
                    inFreeze = true             
                    RLCore.Functions.Notify('Freeze Enabled')               
                end
            end
            ...
            if IsControlJustReleased(0, 47) then
                if lowGrav then
                    lowGrav = false
                    RLCore.Functions.Notify('Low Grav Disabled', 'error')             
                else
                    lowGrav = true              
                    RLCore.Functions.Notify('Low Grav Enabled', 'error')               
                end
            end
