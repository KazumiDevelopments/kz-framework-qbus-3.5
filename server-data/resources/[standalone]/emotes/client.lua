...
RegisterKeyMapping('cancel', "cancel animation", 'keyboard', 'X')
...
AddEventHandler('stuckincar', function(statusupdated) stuckincar = statusupdated end)
...
CreateThread(function()
  TriggerServerEvent("emotes:getkeybinds")
end)
...
RegisterNetEvent('AnimSet:custom');
AddEventHandler('AnimSet:custom', function(anim)
  if anim == nil then
	return
  end
  
  local name = firstToUpper(anim)

  if name == "Reset" or name == "Default" then
    TriggerEvent("AnimSet:default") return
  end
  
  if Walks[name] == nil then
    TriggerEvent('chat:addMessage', {
        template = '<div class="chat-message system"><b>SYSTEM:</b> {0}</div>',
        args = {"'"..name.."' is not a valid walk"}
    })
    return
  else
	local name2 = table.unpack(Walks[name])
	if name2 ~= nil then
		SwitchAnimSet(name2)
    else
        TriggerEvent('chat:addMessage', {
...
RegisterNetEvent("emotes:WalkCommandStart")
AddEventHandler("emotes:WalkCommandStart", function(args)
    WalkCommandStart(args)
end)

RegisterNetEvent("emotes:WalksOnCommand")
AddEventHandler("emotes:WalksOnCommand", function()
    WalksOnCommand()
end)

function WalkCommandStart(args)
  if args[1] == nil then
	WalksOnCommand() return
  end
...
function WalksOnCommand()
  local WalksCommand = ""
  for a in pairsByKeys(Walks) do
    WalksCommand = WalksCommand .. ""..string.lower(a)..", "
  end
  	TriggerEvent('chat:addMessage', {
    template = '<div style="background-color: #8254ff;" class="chat-message"><b>^3Help^0 | </b>{0}<br><br>To reset do /walk reset</div>',
    args = { WalksCommand }
    })
end
...
anims = {
    ...
    ["lighter"] = function(ped)
        ...
        if IsEntityPlayingAnim(ped, testdic, testanim, 3) then
            ...
            local animLength = GetAnimDuration(testdic, testanim)
            TaskPlayAnim(ped, testdic, testanim, 1.0, 1.0, animLength, 48, -1, 0, 0, 0)
        end
        playing_emote = false
    end,
    ...
    ["kickindoor"] = function(ped)
        ...
        if IsEntityPlayingAnim(ped, testdic, testanim, 3) then
            ...
            local animLength = GetAnimDuration(testdic, testanim)
            TaskPlayAnim(ped, testdic, testanim, 1.0, 1.0, animLength, 1, -1, 0, 0, 0)
        end
        Citizen.Wait(1500)
    ...
    ["beg"] = function(ped)
        ...
        if IsEntityPlayingAnim(ped, testdic, testanim, 3) then
            ...
            local animLength = GetAnimDuration(testdic, testanim)
            TaskPlayAnim(ped, testdic, testanim, 1.0, 1.0, animLength, 49, -1, 0, 0, 0)
        end
    end,
    ...
    ["umbrella"] = function(ped)
        if obj ~= 0 then
            ...
            TriggerEvent("destroyPropPerm")
			RLCore.Functions.TriggerCallback('RLCore:HasItem', function(qtty)
				if qtty > 0 then
					loadAnimDict('anim@narcotics@trash')

					TaskPlayAnim(PlayerPedId(), 'anim@narcotics@trash', 'drop_front', 0.9, -8, 2000, 49, 3.0, 0, 0, 0)

					local finished = exports["rl-taskbar"]:taskBar(2500, "Opening Umbrella")
					if finished == 100 then
						...
						if random > 5 then
							obj = CreateObject(GetHashKey("p_amb_brolly_01"), bonePos.x,
											   bonePos.y, bonePos.z + 0.01, true,
											   false, true)
						else
							obj = CreateObject(GetHashKey("p_amb_brolly_01_s"), bonePos.x,
											   bonePos.y, bonePos.z + 0.01, true,
											   false, true)
						end
                    ...
                    RLCore.Functions.Notify("You don't have any umbrella.", 'error')
				end
			end, 'umbrella')
        end
    end,
    ...
    ["smoke"] = function(ped)
		RLCore.Functions.TriggerCallback('RLCore:HasItem', function(qtty)
			if qtty > 0 then
				...
				TriggerServerEvent("RLCore:Server:RemoveItem","ciggy", 1)
				playing_emote = true
            else
                RLCore.Functions.Notify("You don't have any cigarette.", 'error')
			end
		end, 'ciggy')
    end,
    ...
    ["smokefemale"] = function(ped)
		RLCore.Functions.TriggerCallback('RLCore:HasItem', function(qtty)
			if qtty > 0 then
				...
				TriggerEvent("attachItem", "ciggy")
				TriggerServerEvent("RLCore:Server:RemoveItem","cigarette", 1)
			else
				RLCore.Functions.Notify("You don't have any cigarette.", 'error')
			end
		end, 'ciggy')
		...
		RLCore.Functions.TriggerCallback('RLCore:HasItem', function(qtty)
			if qtty > 0 then
				...
				if IsEntityPlayingAnim(ped, animDict, animation, 3) then
					...
					TriggerEvent("destroyProp")
					Wait(2600)
					TriggerEvent("attachItem", "cigar")
				end
				TriggerServerEvent("RLCore:Server:RemoveItem","cigar", 1)
			else
				RLCore.Functions.Notify("You don't have any cigarette.", 'error')
			end
		end, 'cigar')
...
function StressTest(animName)
    if (animName == "pushups" or animName == "situps" or animName == "chinups" or
        ...
        (#(GetEntityCoords(PlayerPedId()) - vector3(1744.53, 2646.94, 45.61)) <
            ...
            10.0) then
        local finished =
            exports["rl-taskbar"]:taskBar(15000, "Relieving Stress")
        if finished == 100 then
...
RegisterNetEvent("emotes:emotesOnCommand")
AddEventHandler("emotes:emotesOnCommand", function()
    emotesOnCommand()
end)
...
function emotesOnCommand()
  	TriggerEvent('chat:addMessage', {
    template = '<div style="background-color: #8254ff;" class="chat-message"><b>^3Help^0 | </b>{0} and ...<br>/e emote name</div>',
    args = { animlist }
    })
end
