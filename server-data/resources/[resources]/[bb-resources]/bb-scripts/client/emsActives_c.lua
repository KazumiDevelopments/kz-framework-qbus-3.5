...
RegisterNetEvent('bb-emsActives:client:createNUI')
AddEventHandler('bb-emsActives:client:createNUI', function(data, draggable)
    local html = ""
...
AddEventHandler("RLCore:Client:OnJobUpdate", function(JobInfo)
    TriggerServerEvent('bb-emsActives:server:updateParamedics')
...
RegisterNetEvent('bb-emsActives:client:updateParamedics')
AddEventHandler('bb-emsActives:client:updateParamedics', function(data)
    if not RLCore or not RLCore.Functions.GetPlayerData().job or RLCore.Functions.GetPlayerData().job.name ~= 'ambulance' then
        return
    end
