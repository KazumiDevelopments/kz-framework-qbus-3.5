...
Citizen.CreateThread(function()
	...
	while true do
		...
		if pumpDistance < 2.5 then
			...
			currentCash = RLCore.Functions.GetPlayerData().money["cash"]
		else
			isNearPump = false
...
AddEventHandler('fuel:refuelFromPump', function(pumpObject, ped, vehicle)
	...
	while isFueling do
		...
		if pumpObject then
			...
			RLCore.Functions.DrawText3D(stringCoords.x, stringCoords.y, stringCoords.z + 1.2, Config.Strings.CancelFuelingPump .. extraString)
			RLCore.Functions.DrawText3D(vehicleCoords.x, vehicleCoords.y, vehicleCoords.z + 0.5, Round(currentFuel, 1) .. "%")
		end
					...
					if not DoesEntityExist(GetPedInVehicleSeat(vehicle, -1)) then
						...
						if GetVehicleFuelLevel(vehicle) < 95 and canFuel then
							if currentCash > 0 then
								RLCore.Functions.DrawText3D(stringCoords.x, stringCoords.y, stringCoords.z + 1.2, Config.Strings.EToRefuel)
								...
								RLCore.Functions.DrawText3D(stringCoords.x, stringCoords.y, stringCoords.z + 1.2, Config.Strings.NotEnoughCash)
							end
						else
							RLCore.Functions.DrawText3D(stringCoords.x, stringCoords.y, stringCoords.z + 1.2, Config.Strings.FullTank)
						end
					end
				else
					Citizen.Wait(250)
