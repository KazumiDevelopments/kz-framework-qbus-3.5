...
$(document).on('click', '#submit-spawn', function(evt){
    ...
    if (spawnType !== "appartment") {
        $.post('http://rl-spawn/spawnplayer', JSON.stringify({
            spawnloc: location,
            typeLoc: spawnType
        }));
    } else {
        $.post('http://rl-spawn/chooseAppa', JSON.stringify({
            appType: location,
        }));
    }
});
...
function setupApps(apps) {
    ...
    $(parent).append('<div class="loclabel" id="location" data-location="null" data-type="lab" data-label="Choose an apartment"><p><span id="null">Choose an apartment</span></p></div>')
