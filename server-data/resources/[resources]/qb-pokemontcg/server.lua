...
RLCore.Functions.CreateUseableItem("boosterbox", function(source, item)
	...
	if Player.Functions.RemoveItem(item.name, 1, item.slot, item.info) then
        TriggerClientEvent("Cards:Client:OpenCards", source, item.name)
           ...
           Citizen.Wait(4000)
        TriggerClientEvent('RLCore:Notify', source, 'You got 4 booster packs!')
            Citizen.Wait(1000) 
    end 
end)

RLCore.Functions.CreateCallback("Cards:server:Menu",function(source,cb)
    local player = RLCore.Functions.GetPlayer(source)
    local item = "...."
        if player ~= nil then
...
RLCore.Functions.CreateUseableItem("boosterpack", function(source, item)
    local Player = RLCore.Functions.GetPlayer(source)  
        TriggerClientEvent("Cards:Client:OpenPack", source)  
        Citizen.Wait(4000)
        TriggerClientEvent('RLCore:Notify', source, 'You got 4 cards!')
end)
...
AddEventHandler('Cards:Server:RemoveItem', function()
    ...
    if pack.amount == nil then
        TriggerClientEvent('RLCore:Notify', source, 'You dont have a boosterpack!')
    else
...
AddEventHandler('Cards:Server:rewarditem', function()
   ...
   -- print(randomChance) -- if you wanna see the random chance
    ...
    if card ~= '' then        
        ...
        TriggerClientEvent('RLCore:Notify', source, 'There is a problem in cards!')
    end 
end)
...
AddEventHandler('Cards:Server:GetPokemon', function(pokemon)
    ...
    local pokemonName = RLCore.Shared.Items[pokemon].label
    if pokemon ~= nil then
        TriggerClientEvent("inventory:client:ItemBox", RLCore.Shared.Items[pokemon], "add")
        TriggerClientEvent('RLCore:Notify', source, "You got "..pokemonName.. "")
            ...
            for k, v in pairs(Config.Badge[type].cards) do 
                if Player.Functions.GetItemByName(k) ~= nil then 
                    if Player.Functions.GetItemByName(k).amount < v then 
                       canBadge = false
                      TriggerClientEvent('RLCore:Notify', source, 'Come back when you have all the items for the '..Config.Badge[type].label, 'error', 5000) 
                    end
                else 
                    canBadge = false
                    TriggerClientEvent('RLCore:Notify', source, 'Come back when you have all the items for the '..Config.Badge[type].label, 'error', 5000)
                ...
                if canBadge then 
                    TriggerClientEvent('RLCore:Notify', source, 'You got a '..Config.Badge[type].label..'!', 'success', 10000)
                    for k, v in pairs(Config.Badge[type].cards) do
...
RLCore.Functions.CreateUseableItem("pokebox", function(source, item)
    local Player = RLCore.Functions.GetPlayer(source)
...
AddEventHandler("Cards:sellItem", function(itemName, amount, price)
    ...
    if xPlayer.Functions.RemoveItem(itemName, amount) then
        ...
        TriggerClientEvent("RLCore:Notify", source, "You sold " .. amount .. " " .. itemName .. " for $" .. price, "success", 5000)
    end
end)

RLCore.Functions.CreateCallback('Cards:server:get:drugs:items', function(source, cb)
    local src = source
