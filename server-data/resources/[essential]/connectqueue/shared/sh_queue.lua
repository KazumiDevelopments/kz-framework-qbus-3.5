    ...
    deferrals.defer()
     ...
     for k, v in pairs(ids) do
        if string.sub(v, 1, string.len("discord:")) == "discord:" then
            identifierDiscord = v
            --print("got discord mert")
        end
        if string.sub(v, 1, string.len("steam:")) == "steam:" then
            identifierSteam = v
            --print("got steam mert")
        end
    end

    if identifierDiscord then
        for k,v in pairs(Config.donorLevels) do
            Citizen.Wait(350)
            if exports.discord_perms:IsRolePresent(src, Config.donorLevels[k].Role) then
                --print("adding prio mert")
                -- _Queue.Priority(identifierSteam, Config.donorLevels[k].Priority)
                _Queue.Priority[string_lower(identifierSteam)] = Config.donorLevels[k].Priority
                break
            end
        end
    end
