...
CreateThread(function()
    while true do
        ...
        if PlayerData and PlayerData.job.name == 'taxi' then
			local boss = Config.Markers['boss']
			local stash = Config.Markers['stash']
...
RegisterNetEvent('rl-taxi:client:DoTaxiNpc')
AddEventHandler('rl-taxi:client:DoTaxiNpc', function()
    if whitelistedVehicle() then
        if NpcData.CountDown == 180 then
            if not NpcData.Active then
                ...
                RLCore.Functions.Notify('The NPC is indicated on your navigation!', 'success')
                    ...
                    while not NpcData.NpcTaken do
                        ...
                        if dist < 20 then
                            ...
                            if dist < 5 then
                                ...
                                DrawText3D(Config.NPCLocations.TakeLocations[NpcData.CurrentNpc].x, Config.NPCLocations.TakeLocations[NpcData.CurrentNpc].y, Config.NPCLocations.TakeLocations[NpcData.CurrentNpc].z, '[E] NPC calls')
                                if IsControlJustPressed(0, Keys["E"]) then
                                    ...
                                    RLCore.Functions.Notify('Bring the NPC to the specified location.')
                                    if NpcData.NpcBlip ~= nil then
                                        RemoveBlip(NpcData.NpcBlip)
                                    end
                                    GetDeliveryLocation()
                ...
                RLCore.Functions.Notify('You\'re already doing an NPC mission..')
            end
        else
            RLCore.Functions.Notify('No NPCs are available..')
        end
    else
        RLCore.Functions.Notify('You\'re not in a Taxi :(')
    end
end)
    ...
    Citizen.CreateThread(function()
        while true do
            ...
            if dist < 20 then
                ...
                if dist < 5 then
                    local npccoords = GetEntityCoords(NpcData.Npc)
                    DrawText3D(Config.NPCLocations.DeliverLocations[NpcData.CurrentDeliver].x, Config.NPCLocations.DeliverLocations[NpcData.CurrentDeliver].y, Config.NPCLocations.DeliverLocations[NpcData.CurrentDeliver].z, '[E] NPC deliver')
                    if IsControlJustPressed(0, Keys["E"]) then
                        local veh = GetVehiclePedIsIn(ped, 0)
                        ...
                        TriggerServerEvent('rl-taxi:server:NpcPay', meterData.currentFare)
                        RLCore.Functions.Notify('Person delivered..', 'success')
                        if NpcData.DeliveryBlip ~= nil then
                            RemoveBlip(NpcData.DeliveryBlip)
                        end
...
RegisterCommand('ttt', function(arg1, arg2, arg3)
    isLoggedIn = true
    PlayerData = RLCore.Functions.GetPlayerData()
end)
                    ...
                    if vehDist < 30 then
                        ...
                        if vehDist < 1.5 then
                            if whitelistedVehicle() then
                                ...
                                DrawText3D(Config.Locations["vehicle"]["x"], Config.Locations["vehicle"]["y"], Config.Locations["vehicle"]["z"] + 0.3, '[E] Vehicle Packs')
                                if IsControlJustReleased(0, Keys["E"]) then
                                    TaxiGarage()
...
AddEventHandler('rl-taxi:client:toggleMeter', function()
    ...
    if IsPedInAnyVehicle(ped, false) then
        if whitelistedVehicle() then
            ...
            RLCore.Functions.Notify('This vehicle does not have a Taxi Meter..', 'error')
        end
    else
        RLCore.Functions.Notify('You\'re not in a vehicle..', 'error')
    end
end)
...
AddEventHandler('rl-taxi:client:enableMeter', function()
    ...
    if meterIsOpen then
        ...
        RLCore.Functions.Notify('The Taxi Meter is not active..', 'error')
    end
end)
...
AddEventHandler('rl-taxi:client:toggleMuis', function()
    Citizen.Wait(400)
    if meterIsOpen then
        ...
        RLCore.Functions.Notify('No Taxi Meter to confess..', 'error')
    end
end)
...
function TakeVehicle(k)
    ...
    RLCore.Functions.SpawnVehicle(Config.AllowedVehicles[k].model, function(veh)
