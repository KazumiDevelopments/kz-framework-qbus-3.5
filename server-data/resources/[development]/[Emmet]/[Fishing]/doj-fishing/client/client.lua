	...
	while true do
		...
		if fishing then
			...
			if IsControlJustReleased(0, Config.StopFishing) then
				endFishing()
				RLCore.Functions.Notify('You Stopped Fishing', 'error')
			end
			...
			if fishing then
				...
				if GetWaterHeight(pos.x, pos.y, pos.z-2, pos.z -3.0)  then
					...
					endFishing()
					RLCore.Functions.Notify('Water isnt deep enough to fish', 'error')
				end
				if IsEntityDead(playerPed) or IsEntityInWater(playerPed) then
					endFishing()
					RLCore.Functions.Notify('Fishing ended', 'error')
				end
			end
			...
			if pause and input ~= 0 then
				...
				if input == correct then
					...
					RLCore.Functions.Notify('The Fish Escaped!', 'error')
...
RegisterNetEvent('fishing:client:attemptTreasureChest', function()
	...
	RLCore.Functions.TriggerCallback('RLCore:HasItem', function(HasItem)
		if HasItem then
			RLCore.Functions.Progressbar("accepted_key", "Inserting Key..", (math.random(2000, 5000)), false, true, {
		  ...
		  RLCore.Functions.Notify("You dont have a key to this lock!", "error")
		end
	  end, 'fishingkey')
end)
...
RegisterNetEvent('fishing:SkillBar', function(message)
	...
	if Config.Skillbar == "reload-skillbar" then
		...
		if finished ~= 100 then
			RLCore.Functions.Notify('The Fish Got Away!', 'error')
			loseBait()
		else
...
RegisterNetEvent('fishing:fishstart', function()
	...
	local pos = GetEntityCoords(playerPed) 
	if IsPedSwimming(playerPed) then return RLCore.Functions.Notify("You can't be swimming and fishing at the same time.", "error") end 
	if IsPedInAnyVehicle(playerPed) then return RLCore.Functions.Notify("You need to exit your vehicle to start fishing.", "error") end 
	if GetWaterHeight(pos.x, pos.y, pos.z-2, pos.z - 3.0)  then
		local time = 1000
		RLCore.Functions.Notify('Using Fishing Rod', 'primary', time)
		Wait(time)
		exports['textUi']:DrawTextUi('show', "Press [X] to stop fishing at any time")
		fishAnimation()
	else
		RLCore.Functions.Notify('You need to go further away from the shore', 'error')
	end
end, false)
...
RegisterNetEvent('doj:client:ReturnBoat', function(args)
	...
	if IsPedInAnyVehicle(ped) then
		if args == 1 then 
			...
			RLCore.Functions.DeleteVehicle(boat)
...
RegisterNetEvent('doj:client:rentaBoat', function(args)
	...
	RLCore.Functions.TriggerCallback('fishing:server:checkMoney', function(isSuccess)
		if isSuccess then 
			if chance == 10 then
				TriggerServerEvent("fishing:server:addTackleBox")
			end
			if args == 1 then 
				RLCore.Functions.SpawnVehicle(Config.RentalBoat, function(boat)
	...
	if data.location == 1 then 
        if Config.Fish24hours then
            ...
            if GetClockHours() >= 6 and GetClockHours() <= 21 then
                ...
                RLCore.Functions.Notify('Too Late to Fish!, Opened from 6:00am / 21:00pm', 'error')
            end
        end
            ...
            if DoesEntityExist(currVeh) then
                if IsThisModelABoat(vehModel) or IsThisModelAJetski(vehModel) or IsThisModelAnAmphibiousCar(vehModel) or IsThisModelAnAmphibiousQuadbike(vehModel) then
                    if IsBoatAnchoredAndFrozen(currVeh) then
						RLCore.Functions.Notify('Retrieving Anchor', 'success')
                        Wait(2000)
						RLCore.Functions.Notify('Anchor Disabled', 'primary')
                    ...
                    elseif not IsBoatAnchoredAndFrozen(currVeh) and CanAnchorBoatHere(currVeh) and GetEntitySpeed(currVeh) < 3 then
						...
						RLCore.Functions.Notify('Dropping Anchor', 'primary')
                        Wait(2000)
						RLCore.Functions.Notify('Anchor Enabled', 'success')
...
catchAnimation = function()
	...
	TaskPlayAnim(ped, animDict, animName, 1.0, -1.0, 1.0, 0, 0, 0, 48, 0)
	local time = 1750
	RLCore.Functions.Notify('Fish Caught!', 'success', time)
	Wait(time)
	TriggerServerEvent('fishing:server:catch') 
	loseBait()
...
fishAnimation = function()
	RLCore.Functions.TriggerCallback('RLCore:HasItem', function(HasItem)
		if HasItem then
		  ...
		  endFishing()
		  RLCore.Functions.Notify("You dont have any fishing bait", "error")
		end
	end, 'fishbait')
end
...
attemptTreasureChest = function()
	...
	RemoveAnimDict(animDict)
	RLCore.Functions.Notify('Attempting to open Treasure Chest', 'primary', 1500)
	Wait(1500)
...
openedTreasureChest = function()
	if math.random(1,15) == 10 then
		TriggerServerEvent("RLCore:Server:RemoveItem", "fishingkey", 1)
		TriggerEvent("inventory:client:ItemBox", RLCore.Shared.Items["fishingkey"], "remove", 1)
		RLCore.Functions.Notify("The corroded key has snapped", "error", 7500)
	end
	TriggerServerEvent("RLCore:Server:RemoveItem", "fishinglootbig", 1)
	TriggerEvent("inventory:client:ItemBox", RLCore.Shared.Items["fishinglootbig"], "remove", 1)
	RLCore.Functions.Notify("Treasure chest opened! Be sure to collect all of your loot!!", "success", 7500)
	local ShopItems = {} 
