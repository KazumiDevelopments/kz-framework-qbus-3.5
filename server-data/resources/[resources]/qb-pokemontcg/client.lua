...
AddEventHandler("Cards:Client:OpenCards", function() 
    ...
    RLCore.Functions.Progressbar("drink_something", "opening box..", 9500, false, true, {
...
AddEventHandler("Cards:Client:OpenPack", function() 
    ...
    RLCore.Functions.Progressbar("drink_something", "opening pack..", 3000, false, true, {
        disableMovement = false,
        disableCarMovement = false,
        disableMouse = false,
        disableCombat = true,
...
AddEventHandler("Cards:client:UseBox", function()
    ...
    RLCore.Functions.Notify("Box is being opened...", "error")
    RLCore.Functions.Progressbar("use_bag", "Box is being opened", 5000, false, true, {
        disableMovement = false,
        disableCarMovement = false,
        disableMouse = false,
        disableCombat = true,
        ...
        TriggerServerEvent("inventory:server:OpenInventory", "stash", "poke_"..RLCore.Functions.GetPlayerData().citizenid, {maxweight = 0.1, slots = 160})
        TriggerEvent("inventory:client:SetCurrentStash", "poke_"..RLCore.Functions.GetPlayerData().citizenid)
        TriggerServerEvent("InteractSound_SV:PlayOnSource", "snap", 1.2)
        TaskPlayAnim(ped, "clothingshirt", "exit", 8.0, 1.0, -1, 49, 0, 0, 0, 0)
        RLCore.Functions.Notify("Box has been opened successfully", "success")
        Citizen.Wait(10000)
...
Citizen.CreateThread(function()
    while true do 
        ...
        if inshop == true then
            Citizen.Wait(1000)
            RLCore.Functions.TriggerCallback("Cards:server:Menu",function(item,amount)
                print(item,amount)
            end)
        end
    end 
...
menu_button:On('select', function(item)
    ...
    RLCore.Functions.TriggerCallback('Cards:server:get:drugs:items', function(CardsResult)
