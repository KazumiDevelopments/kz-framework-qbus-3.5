				...
				if dist < 15 then
					...
					if dist < 1.5 then
						if not Crypto.Exchange.RebootInfo.state then
							DrawText3Ds(Crypto.Exchange.coords.x, Crypto.Exchange.coords.y, Crypto.Exchange.coords.z, '~g~E~w~ - Insert USB')
							...
							if IsControlJustPressed(0, Keys["E"]) then
								RLCore.Functions.TriggerCallback('rl-crypto:server:HasSticky', function(HasItem)
									if HasItem then
										TriggerEvent("mhacking:show")
										TriggerEvent("mhacking:start", math.random(4, 6), 45, HackingSuccess)
									else
										RLCore.Functions.Notify('You do not have a Cryptostick.', 'error')
									end
								end)
							end
						else
...
function ExchangeSuccess()
	TriggerServerEvent('rl-crypto:server:ExchangeSuccess', math.random(1, 10))
end
...
function ExchangeFail()
	...
	if RemoveChance == LosingNumber then
		TriggerServerEvent('rl-crypto:server:ExchangeFail')
		TriggerServerEvent('rl-crypto:server:SyncReboot')
		-- Crypto.Exchange.RebootInfo.state = true
		-- SystemCrashCooldown()
	end
end

RegisterNetEvent('rl-crypto:client:SyncReboot')
AddEventHandler('rl-crypto:client:SyncReboot', function()
	Crypto.Exchange.RebootInfo.state = true
	SystemCrashCooldown()
end)
	...
	Citizen.CreateThread(function()
		while Crypto.Exchange.RebootInfo.state do
			...
			if (Crypto.Exchange.RebootInfo.percentage + 1) <= 100 then
				...
				TriggerServerEvent('rl-crypto:server:Rebooting', true, Crypto.Exchange.RebootInfo.percentage)
			else
				Crypto.Exchange.RebootInfo.percentage = 0
				Crypto.Exchange.RebootInfo.state = false
				TriggerServerEvent('rl-crypto:server:Rebooting', false, 0)
			end
...
AddEventHandler('RLCore:Client:OnPlayerLoaded', function()
	isLoggedIn = true
	TriggerServerEvent('rl-crypto:server:FetchWorth')
	TriggerServerEvent('rl-crypto:server:GetRebootState')
end)

RegisterNetEvent('rl-crypto:client:UpdateCryptoWorth')
AddEventHandler('rl-crypto:client:UpdateCryptoWorth', function(crypto, amount, history)
...
RegisterNetEvent('rl-crypto:client:GetRebootState')
AddEventHandler('rl-crypto:client:GetRebootState', function(RebootInfo)
