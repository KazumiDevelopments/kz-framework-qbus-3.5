...
function openHudMenu() {
    Inventory.Close();
    $.post('https://rl-inventory/OpenHudMenu');
}
...
//                         //     Inventory.Close();
//                         // }
//                         $.post("https://rl-inventory/UseItem", JSON.stringify({
//                             inventory: "player",
//                             item: item,
//                         }));
//                     }
...
function FormatAttachmentInfo(data) {
    $.post("http://rl-inventory/GetWeaponData", JSON.stringify({
        weapon: data.name,
...
function handleAttachmentDrag() {
    ...
    $(".weapon-attachments-remove").droppable({
        ...
        drop: function (event, ui) {
            $.post('http://rl-inventory/RemoveAttachment', JSON.stringify({
...
$(document).on('click', '#weapon-attachments', function (e) {
    ...
    if (!Inventory.IsWeaponBlocked(ClickedItemData.name)) {
        ...
        $.post('http://rl-inventory/Notify', JSON.stringify({
...
function handleDragDrop() {
    ...
    $("#item-drop").droppable({
        ...
        drop: function (event, ui) {
            ...
            $.post("http://rl-inventory/DropItem", JSON.stringify({
                inventory: fromInventory,
                item: fromData,
                amount: parseInt(amount),
            }));
...
$(document).on('click', '.CombineItem', function (e) {
    ...
    if (combineslotData.toData.combinable.anim != null) {
        $.post('http://rl-inventory/combineWithAnim', JSON.stringify({
        ...
        $.post('http://rl-inventory/combineItem', JSON.stringify({
...
function optionSwitch($fromSlot, $toSlot, $fromInv, $toInv, $toAmount, toData, fromData) {
    ...
    $.post("http://rl-inventory/SetInventoryData", JSON.stringify({
...
function swap($fromSlot, $toSlot, $fromInv, $toInv, $toAmount) {
    ...
    if (fromData !== undefined && fromData.amount >= $toAmount) {
        ...
        if (
            ...
            $.post("https://rl-inventory/PlayDropSound", JSON.stringify({}));
            $.post(
                "https://rl-inventory/SetInventoryData",
                JSON.stringify({
                    fromInventory: $fromInv.attr("data-inventory"),
                    toInventory: $toInv.attr("data-inventory"),
                    fromSlot: $fromSlot,
            ...
            if (fromData.amount == $toAmount) {
                if (
                    ...
                    $.post(
                        "https://rl-inventory/getCombineItem",
...
function InventoryError($elinv, $elslot) {
    ...
    $.post("https://rl-inventory/PlayDropFail", JSON.stringify({}));
}
...
(() => {
    ...
    Inventory.Error = function () {
        $.post("http://rl-inventory/PlayDropFail", JSON.stringify({}));
    }
    ...
    Inventory.Close = function () {
        ...
        $.post("http://rl-inventory/CloseInventory", JSON.stringify({}));
    ...
    Inventory.Clickables = function () {
        ...
        $('.cloth').find('img').click(function () {
            $.post("http://rl-inventory/ChangeComponent", JSON.stringify({
    ...
    Inventory.Necessary = function () {
        ...
        $('#clothing-menu').click(function () {
            if (isClothOpen == false) {
                isClothOpen = true
                $.post("http://rl-inventory/OpenClothMenu", JSON.stringify({ delete: isClothOpen }));
...
$(document).on('click', '#rob-money', function (e) {
    ...
    $.post('http://rl-inventory/RobMoney', JSON.stringify({
        TargetId: TargetId
    }));
    $("#rob-money").remove();
});
