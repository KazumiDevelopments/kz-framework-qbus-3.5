				 ...
				 if IsControlJustPressed(0, Keys['E']) then
				  if not Bezig then
					...
					elseif Config.JobData['locations'][k]['name'] == 'GiveTaco' then
						...
						RLCore.Functions.Notify("Je bent nog met iets bezig man bro..", "error")
					end
				end
			end
		end
...
function GetLettuce()
	...
	RLCore.Functions.Progressbar("pickup_sla", "Grabbing lettuce..", 4000, false, true, {
		disableMovement = true,
		disableCarMovement = false,
		disableMouse = false,
		disableCombat = false,
...
function TakeMoney()
	if Config.JobData['register'] >= 10000 then
		...
		RLCore.Functions.Progressbar("search_register", "Taking Money", lockpickTime, false, true, {
            disableMovement = true,
