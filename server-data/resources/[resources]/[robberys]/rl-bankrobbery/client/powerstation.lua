        ...
        if dist < 1.5 then
            if CurrentCops >= Config.MinimumThermitePolice then
                if not Config.PowerStations[closestStation].hit then
                    ...
                    RLCore.Functions.Notify("It seems that the fuses have blown", "error")
                end
            else
...
RegisterNetEvent('rl-bankrobbery:client:SetStationStatus')
AddEventHandler('rl-bankrobbery:client:SetStationStatus', function(key, isHit)
...
RegisterNUICallback('thermitesuccess', function()
    ...
    if currentStation ~= 0 then
        RLCore.Functions.Notify("The fuses are broken", "success")
        TriggerServerEvent("rl-bankrobbery:server:SetStationStatus", currentStation, true)
    elseif currentGate ~= 0 then
        RLCore.Functions.Notify("The door is burned open", "success")
        TriggerServerEvent('rl-doorlock:server:updateState', currentGate, false)
        currentGate = 0
    end
end)
