...
$(document).on('click', '#accept-mail', function(e){
    ...
    $.post('http://rl-phone/AcceptMailButton', JSON.stringify({
...
$(document).on('click', '#remove-mail', function(e){
    e.preventDefault();
    var MailData = $("#"+OpenedMail).data('MailData');
    $.post('http://rl-phone/RemoveMail', JSON.stringify({
        mailId: MailData.mailid
    }));
    $(".mail-home").animate({
        left: 0+"vh"
...
RL.Phone.Functions.SetupMails = function(Mails) {
    ...
    $("#mail-header-mail").html(RL.Phone.Data.PlayerData.charinfo.firstname+"."+RL.Phone.Data.PlayerData.charinfo.lastname+"@ConnectRP.com");
...
$(document).on('click', '#new-advert-submit', function(e){
    ...
    if (Advert !== "") {
        ...
        $.post('http://rl-phone/PostAdvert', JSON.stringify({
            message: Advert,
        }));
    } else {
        RL.Phone.Notifications.Add("fas fa-ad", "Advertisement", "You cannot place an empty AD!", "#ff8f1a", 2000);
    }
});
