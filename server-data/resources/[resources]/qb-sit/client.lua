...
Citizen.CreateThread(function()
	while true do
		...
		if IsControlJustPressed(0, 38) and IsInputDisabled(0) and IsPedOnFoot(playerPed) then
			if sitting then
				wakeup()
			end			
		end
...
RegisterNetEvent("rl-Sit:Sit", function()
	local playerPed = PlayerPedId()
...
function wakeup()
	...
	TriggerServerEvent('rl-sit:leavePlace', currentSitCoords)
...
function sit(object, modelName, data)
	...
	RLCore.Functions.TriggerCallback('rl-sit:getPlace', function(occupied)
		if occupied then
			RLCore.Functions.Notify('There is someone on this chair', 'error')
		else
			local playerPed = PlayerPedId()
			lastPos, currentSitCoords = GetEntityCoords(playerPed), objectCoords

			TriggerServerEvent('rl-sit:takePlace', objectCoords)
