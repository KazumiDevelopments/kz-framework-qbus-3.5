...
$('document').ready(function() {
    ...
    document.onkeyup = function (data) {
        if (data.which == 116 ) {
            $.post('http://rl-houses/toggleCursor');
        }

        if (data.which == 13 ) {
            if (selectedObjectData != null) {
                $.post('http://rl-houses/editOwnedObject', JSON.stringify({
                    objectData: selectedObjectData
                }));
                selectedObjectData = null;
                $.post('http://rl-houses/setupMyObjects', JSON.stringify({}), function(myObjects){
                    $('.decorate-items').html("");
                ...
                if (objData != null) {
                    $.post("http://rl-houses/spawnobject", JSON.stringify({
...
$(document).on('click', '.header-btn', function(){
    ...
    $.post('http://rl-houses/ResetSelectedProp');
...
$(document).on('click', '.footer-btn', function(){
    ...
    if (selectedCategory != "remove-owned-obj") {
        ...
        $(".decorate-footer-buttons").html("");
        $(selectedHeaderButton).removeClass('header-btn-selected');
        $.post('http://rl-houses/deleteSelectedObject');
        $(".decorate-footer-buttons").fadeOut(150);
        $(".decorate-items").fadeOut(150);
    }
});
...
$(document).on('click', '#buy-object', function(){
    $.post('http://rl-houses/buySelectedObject');
    selectedObjectData = null;
    $(".decorate-confirm").css("display", "none");
});

$(document).on('click', '#cancel-object', function(){
    $.post('http://rl-houses/cancelSelectedObject');
    selectedObjectData = null;
    $(".decorate-confirm").css("display", "none");
});
...
$(document).on('click', '.decorate-item', function(){
    ...
    if ($("#"+objId).data('type') == "newObject") {
        if (selectedObject == this) {
            ...
            selectedObject = null;
            $.post('http://rl-houses/removeObject');
        } else {
            $(this).addClass('selected-object');
            selectedObject = this;
            $.post("http://rl-houses/chooseobject", JSON.stringify({
                object: objData.object,
            }));
        ...
        if (selectedObject == this) {
            $(this).removeClass('selected-object');
            $.post('http://rl-houses/deselectOwnedObject')
            selectedObject = null;
        } else {
            $(this).addClass('selected-object');
            selectedObject = this;
            selectedObjectData = myObjectData;
            $.post('http://rl-houses/selectOwnedObject', JSON.stringify({
                objectData: myObjectData
            }))
            $(".decorate-footer-buttons").html("");
            var elem = '<div class="footer-btn" id="remove-owned-obj"><p>Delete</p></div>';
            $(".decorate-footer-buttons").append(elem);
            $(".decorate-footer-buttons").fadeIn(150);
        }
    }
...
Decorations.Close = function() {
    ...
    selectedObjectData = null;
    $.post("http://rl-houses/closedecorations", JSON.stringify({}));
}

$(".property-accept").click(function(e){
    $.post('http://rl-houses/buy', JSON.stringify({}))
});

$(".property-cancel").click(function(e){
    $.post('http://rl-houses/exit', JSON.stringify({}));
});