...
Databank.Close = function() {
    ...
    $.post("http://rl-police/closeDatabank", JSON.stringify({}));
}
...
Fingerprint.Close = function() {
    ...
    $.post('http://rl-police/closeFingerprint');
}

Fingerprint.Update = function(data) {
    $(".fingerprint-id").html("Fingerprint ID<p>"+data.fingerprintId+"</p><br><p>"+data.name+"</p>");
}

$(document).on('click', '.take-fingerprint', function(){
    $.post('http://rl-police/doFingerScan');
})
