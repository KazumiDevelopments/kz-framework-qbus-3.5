...
$(document).on('click', '.racing-race', function(e){
    ...
    var IsRacer = IsInRace(RL.Phone.Data.PlayerData.citizenid, RaceData.RaceData.Racers)
    ...
    if (!RaceData.RaceData.Started || IsRacer) {
        ...
        RL.Phone.Notifications.Add("fas fa-flag-checkered", "Racing", "The race is already ongoing.", "#1DA1F2");
    }
});
...
function SetupRaces(Races) {
    ...
    if (Races.length > 0) {
        ...
        $.each(Races, function(i, race){
            ...
            var InRace = IsInRace(RL.Phone.Data.PlayerData.citizenid, race.RaceData.Racers);
            var Creator = IsCreator(RL.Phone.Data.PlayerData.citizenid, race);
...
$(document).on('click', '#join-race', function(e){
    ...
    $.post('http://rl-phone/IsInRace', JSON.stringify({}), function(IsInRace){
        if (!IsInRace) {
            $.post('http://rl-phone/RaceDistanceCheck', JSON.stringify({
                RaceId: Data.RaceId,
                Joined: true,
            }), function(InDistance){
                if (InDistance) {
                    $.post('http://rl-phone/IsBusyCheck', JSON.stringify({
                        check: "editor"
                    }), function(IsBusy){
                        if (!IsBusy) {
                            $.post('http://rl-phone/JoinRace', JSON.stringify({
                                RaceData: Data,
                            }));
                            ...
                            RL.Phone.Notifications.Add("fas fa-flag-checkered", "Racing", "You are in an editor.", "#1DA1F2");
                        }
                    });
                }
            })
        } else {
            RL.Phone.Notifications.Add("fas fa-flag-checkered", "Racing", "You are already in a race.", "#1DA1F2");
        }
    });
});
...
$(document).on('click', '#quit-race', function(e){
    ...
    $.post('http://rl-phone/LeaveRace', JSON.stringify({
        RaceData: Data,
    }));
...
$(document).on('click', '#start-race', function(e){
    ...
    $.post('http://rl-phone/StartRace', JSON.stringify({
        RaceData: Data,
    }));
...
$(document).on('click', '.dropdown .dropdown-menu li', function(e) {
    $.post('http://rl-phone/GetTrackData', JSON.stringify({
        RaceId: $(this).attr('id')
    }), function(TrackData){
...
$(document).on('click', '#setup-race', function(e){
    ...
    $.post('http://rl-phone/GetRaces', JSON.stringify({}), function(Races){
...
$(document).on('click', '#create-race', function(e){
    e.preventDefault();
    $.post('http://rl-phone/IsAuthorizedToCreateRaces', JSON.stringify({}), function(data){
            ...
            if (!data.IsBusy) {
                $.post('http://rl-phone/IsBusyCheck', JSON.stringify({
                    ...
                    if (!InRace) {
                        ...
                        RL.Phone.Notifications.Add("fas fa-flag-checkered", "Racing", "You are in a race.", "#1DA1F2");
                    }
                });
            } else {
                RL.Phone.Notifications.Add("fas fa-flag-checkered", "Racing", "You are already creating a Track.", "#1DA1F2");
            }
        } else {
            RL.Phone.Notifications.Add("fas fa-flag-checkered", "Racing", "You have no rights to create Race Tracks.", "#1DA1F2");
        }
    });
});
...
$(document).on('click', '#racing-create-accept', function(e){
    ...
    if (TrackName !== "" && TrackName !== undefined && TrackName !== null) {
        $.post('http://rl-phone/IsAuthorizedToCreateRaces', JSON.stringify({
            TrackName: TrackName
        }), function(data){
            if (data.IsAuthorized) {
                if (data.IsNameAvailable) {
                    $.post('http://rl-phone/StartTrackEditor', JSON.stringify({
                        TrackName: TrackName
                    }));
                    ...
                    RL.Phone.Notifications.Add("fas fa-flag-checkered", "Racing", "This name is not available.", "#1DA1F2");
                }
            } else {
                RL.Phone.Notifications.Add("fas fa-flag-checkered", "Racing", "You have no rights to create Race Tracks.", "#1DA1F2");
            }
        ...
        RL.Phone.Notifications.Add("fas fa-flag-checkered", "Racing", "You must enter a Track name.", "#1DA1F2");
    }
});
...
$(document).on('click', '#setup-race-accept', function(e){
    ...
    $.post('http://rl-phone/HasCreatedRace', JSON.stringify({}), function(HasCreatedRace){
        if (!HasCreatedRace) {
            $.post('http://rl-phone/RaceDistanceCheck', JSON.stringify({
                RaceId: track,
                Joined: false,
                ...
                if (InDistance) {
                    if (track !== undefined || track !== null) {
                        if (laps !== "") {
                            $.post('http://rl-phone/CanRaceSetup', JSON.stringify({}), function(CanSetup){
                                if (CanSetup) {
                                    $.post('http://rl-phone/SetupRace', JSON.stringify({
                                        RaceId: track,
                                        AmountOfLaps: laps,
                                    }))
                                    $(".racing-overview").animate({
                                    ...
                                    RL.Phone.Notifications.Add("fas fa-flag-checkered", "Racing", "No races can be made at this time.", "#1DA1F2");
                                }
                            });
                        } else {
                            RL.Phone.Notifications.Add("fas fa-flag-checkered", "Racing", "Enter an amount of rounds.", "#1DA1F2");
                        }
                    } else {
                        RL.Phone.Notifications.Add("fas fa-flag-checkered", "Racing", "You have not chosen a Track.", "#1DA1F2");
                    }
                }
            })
        } else {
            RL.Phone.Notifications.Add("fas fa-flag-checkered", "Racing", "You already have a race active.", "#1DA1F2");
        }
    });
});
...
$(document).on('click', '#leaderboards-race', function(e){
    ...
    $.post('http://rl-phone/GetRacingLeaderboards', JSON.stringify({}), function(Races){
        if (Races !== null) {
