                        ...
                        if not Config.BigBanks["pacific"]["lockers"][k]["isOpened"] then
                            if lockerDist < 5 then
                                ...
                                if lockerDist < 0.5 then
                                    DrawText3Ds(Config.BigBanks["pacific"]["lockers"][k].x, Config.BigBanks["pacific"]["lockers"][k].y, Config.BigBanks["pacific"]["lockers"][k].z + 0.3, '[E] Crack Safe')
    ...
    if dist < 1.5 then
        RLCore.Functions.TriggerCallback('rl-bankrobbery:server:isRobberyActive', function(isBusy)
            if not isBusy then
                                    ...
                                    RLCore.Functions.Progressbar("hack_gate", "Connecting Electronic Kit", 7500, false, true, {
                                        ...
                                        if not copsCalled then
                                            ...
                                            if Config.BigBanks["pacific"]["alarm"] then
                                                TriggerServerEvent("rl-bankrobbery:server:callCops", "pacific", 0)
                ...
                RLCore.Functions.Notify("The security lock is active, the door cannot be opened atm", "error", 5500)
            end
        end)
    end
end)
...
function OnHackPacificDone(success, timeremaining)
    if success then
        ...
        TriggerServerEvent('rl-bankrobbery:server:setBankState', "pacific", true)
    else
		TriggerEvent('mhacking:hide')
	end
end
