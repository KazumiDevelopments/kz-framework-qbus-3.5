                    ...
                    if dist <= 7.5 then
                        if Driver == PlayerPed then
                            if not washingVehicle then
                                DrawText3Ds(Config.Locations[k]["coords"]["x"], Config.Locations[k]["coords"]["y"], Config.Locations[k]["coords"]["z"], '~g~E~w~ Wash car')
                                if IsControlJustPressed(0, Keys["E"]) then
                                    TriggerServerEvent('rl-carwash:server:washCar')
                                end
                            else
...
AddEventHandler('rl-carwash:client:washCar', function()
    ...
    RLCore.Functions.Progressbar("search_cabin", "Vehicle is being washed..", math.random(4000, 8000), false, true, {
        ...
        RLCore.Functions.Notify("Washing canceled..", "error")
        washingVehicle = false
    end)
end)
...
function Progressbar(duration, label)
	local retval = nil
	RLCore.Functions.Progressbar("clean", label, duration, false, false, {
		disableMovement = true,
		disableCarMovement = true,
		disableMouse = false,
		disableCombat = true,
