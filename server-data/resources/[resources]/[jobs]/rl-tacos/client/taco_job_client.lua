            ...
            local Gebied = GetDistanceBetweenCoords(pos.x, pos.y, pos.z, v.x, v.y, v.z, false)
				if Gebied <= 1.5  and JobBusy == false and not IsPedInAnyVehicle(GetPlayerPed(-1), false) then
					...
					if IsControlJustPressed(0, Keys['E']) then
						...
						TriggerServerEvent('rl-taco:server:start:black')
					end
					else
	...
	while true do 
		...
		if Gebied <= 1.5  and Config.JobBusy == false and not IsPedInAnyVehicle(GetPlayerPed(-1), false) then
			...
			if IsControlJustPressed(0, Keys['E']) then
				...
				TriggerServerEvent('rl-tacos:server:get:stuff')
				RLCore.Functions.Notify("Deliver the box to the "..TacoShop[1]["info"], "success", 10000)
				Config.JobBusy = true
			end
...
RegisterNetEvent('rl-taco:start:black:job')
AddEventHandler('rl-taco:start:black:job', function()
	rnd = math.random(1,#DropOffs)
	CreateBlip()
	RLCore.Functions.Notify("Deliver this bag to "..DropOffs[rnd]["info"], "success", 10000)
	TriggerServerEvent('RLCore:Server:AddItem', "taco-bag", 1)
	TriggerEvent("inventory:client:ItemBox", RLCore.Shared.Items["taco-bag"], "add")
	if Tasks then
		return
	end
	Tasks = true
...
RegisterNetEvent('rl-taco:client:set:taco:count')
AddEventHandler('rl-taco:client:set:taco:count', function(what, bool)
	Config.JobData[what] = bool
end)

RegisterNetEvent('rl-taco:client:set:register:count')
AddEventHandler('rl-taco:client:set:register:count', function(bool)
...
function EndJob()
	RLCore.Functions.TriggerCallback('RLCore:HasItem', function(HasItem)
		if JobBusy == true and HasItem then
			...
			Citizen.Wait(800)
			DeleteBlip()
			TriggerServerEvent('rl-taco:server:reward:money', true)
			Config.JobData['register'] = Config.JobData['register'] + math.random(100,200)
			TriggerServerEvent('RLCore:Server:RemoveItem', "taco-bag", 1)
			TriggerEvent("inventory:client:ItemBox", RLCore.Shared.Items["taco-bag"], "remove")
		else
			RLCore.Functions.Notify("You didnt even took the order with you..", "error")
		end 
 	end, 'taco-bag')
end
