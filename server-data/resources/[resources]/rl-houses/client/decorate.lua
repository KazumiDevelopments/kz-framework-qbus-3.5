	...
	while true do
		...
		if DecoMode then
			...
			if GetDistanceBetweenCoords(camPos.x, camPos.y, camPos.z, Config.Houses[closesthouse].coords.enter.x, Config.Houses[closesthouse].coords.enter.y, Config.Houses[closesthouse].coords.enter.z, false) > 50.0 then
				DisableEditMode()
				closeDecorateUI()
				RLCore.Functions.Notify("You have gone out of range!")
			end
		end
	end
end)

RegisterNetEvent("rl-houses:client:decorate")
AddEventHandler("rl-houses:client:decorate", function()
	Citizen.Wait(500)
	if inside then 
		if hasKey then 
			...
			RLCore.Functions.Notify("You must have the keys to the house!", "error")
		end
	else
...
RegisterNetEvent("rl-houses:server:sethousedecorations")
AddEventHandler("rl-houses:server:sethousedecorations", function(house, decorations)
...
function DisableEditMode()
	...
	peanut = false
	DecoMode = false
	TriggerEvent('rl-anticheat:client:ToggleDecorate', false)
end
...
function LoadDecorations(house)
	if Config.Houses[house].decorations == nil or next(Config.Houses[house].decorations) == nil then
		RLCore.Functions.TriggerCallback('rl-houses:server:getHouseDecorations', function(result)
...
function SaveDecorations()
	if closesthouse ~= nil then
		...
		TriggerServerEvent("rl-houses:server:savedecorations", closesthouse, ObjectList)
	end
end
...
function CheckMovementInput()
	...
	if IsControlJustReleased(0, Keys["LEFTSHIFT"]) then
		...
		RLCore.Functions.Notify("Speed is ".. tostring(speeds[curSpeed]))
	end
