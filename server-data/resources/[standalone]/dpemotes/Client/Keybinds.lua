...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        if not IsPedSittingInAnyVehicle(PlayerPedId()) and RLCore then
            for k, v in pairs(Config.KeybindKeys) do
                if IsControlJustPressed(0, v) then
                    local metadata = RLCore.Functions.GetPlayerData().metadata
                        if not metadata["isdead"] and not metadata["inlaststand"] and not metadata["ishandcuffed"] then
