...
Citizen.CreateThread(function()
	...
	while true do
			...
			for i,theKey in pairs(chdata) do
				...
				if theKey.value and WarMenu.Button(theKey.name, theKey.value) then -- make sure we actually have a value and create the button so it can be "pressed"
					...
					if result then
						...
						TriggerServerEvent('bbhandilngsave', chdata, string.lower(GetDisplayNameFromVehicleModel(GetEntityModel(GetVehiclePedIsIn( PlayerPedId(), false )))))
					end
		...
		WarMenu.Display()
		elseif IsControlJustReleased(0, 116) and GetVehiclePedIsIn( PlayerPedId(), false ) ~= 0 then -- Press B, also make sure we are in a vehicle otherwise the script will crash
