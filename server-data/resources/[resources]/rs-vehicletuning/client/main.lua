...
AddEventHandler('RLCore:Client:OnPlayerLoaded', function()
    ...
    isLoggedIn = true
    RLCore.Functions.TriggerCallback('rs-vehicletuning:server:GetAttachedVehicle', function(plates)
        for k, v in pairs(plates) do
            Config.Plates[k].AttachedVehicle = v.AttachedVehicle
        end
    end)

    RLCore.Functions.TriggerCallback('rs-vehicletuning:server:GetDrivingDistances', function(retval)
            ...
            if PlayerJob.name == "mechanic" then
                ...
                if OnDutyDistance < 20 then
                    ...
                    if OnDutyDistance < 1 then
                        if onDuty then
                            DrawText3Ds(Config.Locations["duty"].x, Config.Locations["duty"].y, Config.Locations["duty"].z, "[E] Leaving service")
                        else
                            DrawText3Ds(Config.Locations["duty"].x, Config.Locations["duty"].y, Config.Locations["duty"].z, "[E] Entering service")
                        end
                        if IsControlJustReleased(0, Keys["E"]) then
                            TriggerServerEvent("RLCore:ToggleDuty")
                        end
                                    ...
                                    if IsPedInAnyVehicle(GetPlayerPed(-1)) then
                                        if not IsThisModelABicycle(GetEntityModel(veh)) then
                                            ...
                                            if IsControlJustPressed(0, Config.Keys["E"]) then
                                                ...
                                                Wait(500)
                                                DoScreenFadeIn(250)
                                                RLCore.Functions.TriggerCallback('rs-vehicletuning:server:SetAttachedVehicle', function(result)
                                                end, veh, k)
                                                -- TriggerServerEvent('rs-vehicletuning:server:SetAttachedVehicle', veh, k)
                                            end
                                        else
                                            RLCore.Functions.Notify("You cannot put bicycles on the ramp!", "error")
                                        end
                                    end
                                end
                            end
...
function PartMenu(part)
    ClearMenu()
    Menu.addButton("Repair ("..RLCore.Shared.Items[Config.RepairCostAmount[part].item]["label"].." "..Config.RepairCostAmount[part].costs.."x)", "RepairPart", part)
    Menu.addButton("Back", "VehicleOptions", nil)
    Menu.addButton("Close Menu", "CloseMenu", nil) 
end
...
function RepairPart(part)
    ...
    RLCore.Functions.TriggerCallback('rs-inventory:server:GetStashItems', function(StashItems)
        for k, v in pairs(StashItems) do
            if v.name == PartData.item then
                if v.amount >= PartData.costs then
                    RLCore.Functions.Progressbar("repair_part", Config.ValuesLabels[part].." repairing", math.random(5000, 10000), false, true, {
                        ...
                        RLCore.Functions.Notify("Reparatie Canceled..", "error")
                    end)
                    break
                else
                    RLCore.Functions.Notify('Er zijn niet genoeg materialen in de kluis..', 'error')
                end
                break
            else
                RLCore.Functions.Notify('Er zijn niet genoeg materialen in de kluis..', 'error')
...
AddEventHandler('rs-vehicletuning:client:RepaireeePart', function(part)
    ...
    RLCore.Functions.Notify("De "..Config.ValuesLabels[part].." is gerepareerd!")
end)
    ...
    while true do 
        ...
        if (IsPedInAnyVehicle(GetPlayerPed(-1), false)) then
            ...
            if not IsThisModelABicycle(GetEntityModel(veh)) and GetPedInVehicleSeat(veh, -1) == GetPlayerPed(-1) then
                ...
                local plate = GetVehicleNumberPlateText(veh)
                if VehicleStatus[plate] == nil then 
                    RLCore.Functions.TriggerCallback('vehiclemod:server:setupVehicleStatus', function(result)
            ...
            if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, vehpos.x, vehpos.y, vehpos.z, true) < 5.0 then
                if not IsThisModelABicycle(GetEntityModel(veh)) then
                    ...
                    if VehicleStatus[plate] ~= nil then 
                        ...
                        RLCore.Functions.Notify("Geen status bekend..", "error")
                    end
                else
                    RLCore.Functions.Notify("Geen geldig voertuig..", "error")
                end
            else
                RLCore.Functions.Notify("Je staat niet dicht genoeg bij het voertuig..", "error")
            end
        else
            RLCore.Functions.Notify("Je moet eerst in het voertuig zitten..", "error")
        end
    else
        RLCore.Functions.Notify("Je moet buiten het voertuig staan..", "error")
    end
end)
...
AddEventHandler('vehiclemod:client:fixEverything', function()
    if (IsPedInAnyVehicle(GetPlayerPed(-1), false)) then
        ...
        RLCore.Functions.Notify("Je zit niet in een voertuig..", "error")
    end
end)
                ...
                if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, vehpos.x, vehpos.y, vehpos.z, true) < 5.0 then
                    if not IsThisModelABicycle(GetEntityModel(veh)) then
                        ...
                        if VehicleStatus[plate] ~= nil and VehicleStatus[plate][part] ~= nil then
                            ...
                            RLCore.Functions.Progressbar("repair_advanced", "Voertuig repareren..", lockpickTime, false, true, {
                                disableMovement = true,
                                disableCarMovement = true,
                                disableMouse = false,
                                disableCombat = true,
                            ...
                            RLCore.Functions.Notify("Geen geldige onderdeel..", "error")
                        end
                    else
                        RLCore.Functions.Notify("Geen geldig voertuig..", "error")
                    end
...
function SendStatusMessage(statusList)
    if statusList ~= nil then 
        TriggerEvent('chat:addMessage', {
            ...
            args = {'Voertuig Status', round(statusList["engine"]) .. "/" .. Config.MaxStatusValues["engine"] .. " ("..RLCore.Shared.Items["advancedrepairkit"]["label"]..")", round(statusList["body"]) .. "/" .. Config.MaxStatusValues["body"] .. " ("..RLCore.Shared.Items[Config.RepairCost["body"]]["label"]..")", round(statusList["radiator"]) .. "/" .. Config.MaxStatusValues["radiator"] .. ".0 ("..RLCore.Shared.Items[Config.RepairCost["radiator"]]["label"]..")", round(statusList["axle"]) .. "/" .. Config.MaxStatusValues["axle"] .. ".0 ("..RLCore.Shared.Items[Config.RepairCost["axle"]]["label"]..")", round(statusList["brakes"]) .. "/" .. Config.MaxStatusValues["brakes"] .. ".0 ("..RLCore.Shared.Items[Config.RepairCost["brakes"]]["label"]..")", round(statusList["clutch"]) .. "/" .. Config.MaxStatusValues["clutch"] .. ".0 ("..RLCore.Shared.Items[Config.RepairCost["clutch"]]["label"]..")", round(statusList["fuel"]) .. "/" .. Config.MaxStatusValues["fuel"] .. ".0 ("..RLCore.Shared.Items[Config.RepairCost["fuel"]]["label"]..")"}
        })
    end
end
