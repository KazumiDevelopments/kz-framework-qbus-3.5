...
$(document).on('click', '.house-adress-location', function(e){
    ...
    $.post('http://rl-phone/SetGPSLocation', JSON.stringify({
...
$(document).on('click', '.appartment-adress-location', function(e){
    ...
    $.post('http://rl-phone/SetApartmentLocation', JSON.stringify({
...
$(document).on('click', '.person-search-result-apartment > span', function(e){
    ...
    RL.Phone.Notifications.Add("fas fa-university", "MEOS", "House No. copied!", "#badc58", 1750);
...
$(document).on('click', '.confirm-search-person-test', function(e){
    ...
    if (SearchName !== "") {
        $.post('http://rl-phone/FetchSearchResults', JSON.stringify({
            input: SearchName,
        }), function(result){
            if (result != null) {
                $(".person-search-results").html("");
                ...
                RL.Phone.Notifications.Add("politie", "MEOS", "There are no search results!");
                $(".person-search-results").html("");
            }
        });
    } else {
...
$(document).on('click', '.confirm-search-person-house', function(e){
    ...
    if (SearchName !== "") {
        $.post('http://rl-phone/FetchPlayerHouses', JSON.stringify({
            input: SearchName,
        }), function(result){
            if (result != null) {
                $(".person-search-results").html("");
...
$(document).on('click', '.confirm-search-vehicle', function(e){
    ...
    if (SearchName !== "") {
        $.post('http://rl-phone/FetchVehicleResults', JSON.stringify({
            input: SearchName,
        }), function(result){
            if (result != null) {
                $(".vehicle-search-results").html("");
...
$(document).on('click', '.scan-search-vehicle', function(e){
    e.preventDefault();
    $.post('http://rl-phone/FetchVehicleScan', JSON.stringify({}), function(vehicle){
        if (vehicle != null) {
            $(".vehicle-search-results").html("");
            var APK = "Ja";
            if (!vehicle.status) {
            ...
            RL.Phone.Notifications.Add("politie", "MEOS", "No vehicle around!");
...
$(document).on('click', '.meos-recent-alert', function(e){
    ...
    if (alertData.coords != undefined && alertData.coords != null) {
        $.post('http://rl-phone/SetAlertWaypoint', JSON.stringify({
            alert: alertData,
        }));
    } else {
        RL.Phone.Notifications.Add("politie", "MEOS", "This message has no GPS Location!");
    }
});
...
$(document).on('click', '.meos-clear-alerts', function(e){
    ...
    RL.Phone.Notifications.Add("politie", "MEOS", "All notifications have been removed!");
});