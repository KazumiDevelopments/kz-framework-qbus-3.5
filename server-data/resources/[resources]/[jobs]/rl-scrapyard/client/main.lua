...
AddEventHandler("RLCore:Client:OnPlayerLoaded", function()
    TriggerServerEvent("rl-scrapyard:server:LoadVehicleList")
end)
							...
							if IsControlJustReleased(0, Keys["E"]) then
								if GetPedInVehicleSeat(vehicle, -1) == GetPlayerPed(-1) then
									if IsVehicleValid(GetEntityModel(vehicle)) then 
										...
										RLCore.Functions.Notify("This vehicle cannot be scrapped..", "error")
									end
								else
									RLCore.Functions.Notify("You are not the driver..", "error")
								end
							end
						end
					end
...
RegisterNetEvent('rl-scapyard:client:setNewVehicles')
AddEventHandler('rl-scapyard:client:setNewVehicles', function(vehicleList)
	...
	if Config.CurrentVehicles ~= nil and next(Config.CurrentVehicles) ~= nil then 
		...
		for k, v in pairs(Config.CurrentVehicles) do
			if Config.CurrentVehicles[k] ~= nil then 
				local vehicleInfo = RLCore.Shared.Vehicles[v]
		...
		TriggerServerEvent('rl-phone:server:sendNewMail', {
            sender = "Scrapyard Cooperation",
            subject = "Vehicle list",
            message = "You can only demolish a few vehicles from me. <br /> You can keep everything you demolish from me as long as you dont bother me. <br /> <br /> <strong> Vehicle list:</strong><br />".. vehicleList,
            button = {}
		})
		...
		RLCore.Functions.Notify("You are not allowed to demolish vehicles now..", "error")
	end
end
...
function ScrapVehicle(vehicle)
	...
	RLCore.Functions.Progressbar("scrap_vehicle", "Destroying vehicle..", scrapTime, false, true, {
