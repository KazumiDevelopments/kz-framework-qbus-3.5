        ...
        if Races ~= nil then
                ...
                for k, race in pairs(Races) do
                    if Races[k] ~= nil then
                        if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, Races[k].startx, Races[k].starty, Races[k].startz, true) < 15.0 and not Races[k].started then
                            DrawText3Ds(Races[k].startx, Races[k].starty, Races[k].startz, "[~g~H~w~] Participate in the Race (~g~$"..Races[k].amount..",-~w~)")
                            if IsControlJustReleased(0, Keys["H"]) then
                                TriggerServerEvent("rl-streetraces:JoinRace", k)
                                TriggerEvent("debug", 'Races: Join Race', 'success')
                            end
                        end
                    end
            ...
            if RaceId ~= 0 and not InRace then
                if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, Races[RaceId].startx, Races[RaceId].starty, Races[RaceId].startz, true) < 15.0 and not Races[RaceId].started then
                    DrawText3Ds(Races[RaceId].startx, Races[RaceId].starty, Races[RaceId].startz, "Race begins shortly...")
                end
            end
            -- In race en gestart
            if RaceId ~= 0 and InRace then
                if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, Races[RaceId].endx, Races[RaceId].endy, pos.z, true) < 250.0 and Races[RaceId].started then
                    ...
                    if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, Races[RaceId].endx, Races[RaceId].endy, pos.z, true) < 15.0 then
                        TriggerServerEvent("rl-streetraces:RaceWon", RaceId)
                        TriggerEvent("debug", 'Races: Won', 'success')
                        InRace = false
                    end
                end
            end
...
RegisterNetEvent('rl-streetraces:StartRace')
AddEventHandler('rl-streetraces:StartRace', function(race)
...
RegisterNetEvent('rl-streetraces:RaceDone')
AddEventHandler('rl-streetraces:RaceDone', function(race, winner)
    if RaceId ~= 0 and RaceId == race then
        RaceId = 0
        InRace = false
        RLCore.Functions.Notify("Race finished! The winner is "..winner.. "!")
    end
end)

RegisterNetEvent('rl-streetraces:StopRace')
AddEventHandler('rl-streetraces:StopRace', function()
    RaceId = 0
    InRace = false
end)
...
AddEventHandler('rl-streetraces:CreateRace', function(amount)
    ...
    if DoesBlipExist(WaypointHandle) then
        ...
        if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, cx, cy, groundZ, true) > 500.0 then
            ...
            TriggerServerEvent("rl-streetraces:NewRace", race)
            RLCore.Functions.Notify("Race made for $"..amount..",-!", "success")
        else
            RLCore.Functions.Notify("End position too close!", "error")
        end
    else
        RLCore.Functions.Notify("You have to put a marker!", "error")
    end
end)

RegisterNetEvent('rl-streetraces:SetRace')
AddEventHandler('rl-streetraces:SetRace', function(RaceTable)
    Races = RaceTable
end)

RegisterNetEvent('rl-streetraces:SetRaceId')
AddEventHandler('rl-streetraces:SetRaceId', function(race)
    RaceId = race
    SetNewWaypoint(Races[RaceId].endx, Races[RaceId].endy)
end)
