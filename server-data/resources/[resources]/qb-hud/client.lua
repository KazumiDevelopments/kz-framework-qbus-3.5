...
local function loadSettings(settings)
    ...
    RLCore.Functions.Notify("HUD Settings Loaded!", 'success')
    Wait(1000)
...
local function restartHud()
    ...
    RLCore.Functions.Notify("HUD Is Restarting!", 'error')
    ...
    Wait(2600)
    RLCore.Functions.Notify("HUD Is Now Started!", 'success')
end
...
RegisterNetEvent("hud:client:resetStorage", function()
    ...
    RLCore.Functions.TriggerCallback('hud:server:getMenu', function(menu) loadSettings(menu); SetResourceKvp('hudSettings', json.encode(menu)) end)
end)
...
RegisterNetEvent("hud:client:LoadMap", function()
    ...
    if Menu.isToggleMapShapeChecked == "square" then
        ...
        if Menu.isMapNotifChecked then
            RLCore.Functions.Notify("Square Map Loading...")
        end
        ...
        Wait(1200)
        if Menu.isMapNotifChecked then
            RLCore.Functions.Notify("Square Map Has Loaded!")
        end
    elseif Menu.isToggleMapShapeChecked == "circle" then
        ...
        if Menu.isMapNotifChecked then
            RLCore.Functions.Notify("Circle Map Loading...")
        end
        ...
        Wait(1200)
        if Menu.isMapNotifChecked then
            RLCore.Functions.Notify("Circle Map Has Loaded!")
        end
    end
end)
...
RegisterNUICallback('cinematicMode', function()
    ...
    if Menu.isCineamticModeChecked then
        ...
        if Menu.isCinematicNotifChecked then
            RLCore.Functions.Notify("Cinematic Mode Off!", 'error')
        end
        DisplayRadar(1)
    else
        CinematicShow(true)
        ...
        if Menu.isCinematicNotifChecked then
            RLCore.Functions.Notify("Cinematic Mode On!")
        end
    end
    TriggerEvent("hud:client:playHudChecklistSound")
    saveSettings()
...
RegisterNetEvent('hud:client:UpdateNeeds', function(newHunger, newThirst) -- Triggered in rl-core
    hunger = newHunger
    thirst = newThirst
end)
...
RegisterNetEvent("rl-admin:client:ToggleDevmode", function()
    dev = not dev
end)
...
RegisterCommand('+engine', function()
    ...
    if GetIsVehicleEngineRunning(vehicle) then
        RLCore.Functions.Notify("Engine Shut Down!")
    else
            ...
            if IsPedInAnyVehicle(ped, false) then
                if exports['lj-fuel']:GetFuel(GetVehiclePedIsIn(ped, false)) <= 20 then -- At 20% Fuel Left
                    if Menu.isLowFuelChecked then
                        ...
                        RLCore.Functions.Notify("Fuel Level Low!", 'error')
...
CreateThread(function()
    while true do
        ...
        if LocalPlayer.state.isLoggedIn then
            local ped = PlayerPedId()
            if IsPedInAnyVehicle(ped, false) then
                RLCore.Functions.TriggerCallback('hud:server:HasHarness', function(hasItem)
                    if hasItem then
                        harness = true
                    else
                        harness = false
