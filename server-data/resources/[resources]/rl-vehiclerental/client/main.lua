...
Citizen.CreateThread(function()
    for k, v in pairs(Config.RentalPoints) do
        ...
        AddTextComponentSubstringPlayerName("Rental - Collection point")
...
RentVehicleMenu = function()
    ...
    for k, v in pairs(Config.RentalVehicles) do
        Menu.addButton(Config.RentalVehicles[k]["label"].." - Deposit Cost: $ ".. Config.RentalVehicles [k] ["price"], "RentVehicle", k)
    end
...
RentVehicle = function(selectedVehicle)
    ...
    TriggerServerEvent('rl-vehiclerental:server:SetVehicleRented', true, Config.RentalVehicles[selectedVehicle])

    RentedVehicleData = selectedVehicle
end

RegisterNetEvent('rl-vehiclerental:server:SpawnRentedVehicle')
...
ReturnVehicle = function()
    ...
    RLCore.Functions.Notify('You do not have a deposit open..', 'error')
end
...
AcceptReturn = function()
    ...
    if noSpace(VehiclePlate) ~= noSpace(RentedVehiclePlate) then
        RLCore.Functions.Notify('This is not a rented vehicle..', 'error')
    else
        TriggerServerEvent('rl-vehiclerental:server:SetVehicleRented', false, Config.RentalVehicles[RentedVehicleData])
        RLCore.Functions.DeleteVehicle(CurrentVehicle)
        RentedVehiclePlate = nil
        RentedVehicleData = nil

        TriggerEvent("debug", 'Rentel: Rented Vehicle Returned', 'success')
    end
    RentalMenu()
end
...
RentalMenu = function()
    ClearMenu()
    Menu.addButton("Rent a vehicle", "RentVehicleMenu", nil)
