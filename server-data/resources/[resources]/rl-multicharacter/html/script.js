    ...
    window.addEventListener('message', function (event) {
        ...
        if (data.action == "ui") {
            if (data.toggle) {
                ...
                setTimeout(function(){
                    $.post('http://rl-multicharacter/setupCharacters');
                    setTimeout(function(){
                        ...
                        $.post('http://rl-multicharacter/removeBlur');
                    }, 2000);
                }, 2000);
            } else {
...
$('.disconnect-btn').click(function(e){
    ...
    $.post('http://rl-multicharacter/closeUI');
    $.post('http://rl-multicharacter/disconnectButton');
});
...
$(document).on('click', '.character', function(e) {
    ...
    if (selectedChar === null) {
        ...
        if ((selectedChar).data('cid') == "") {
            ...
            $.post('http://rl-multicharacter/cDataPed', JSON.stringify({
                cData: cDataPed
            }));
        } else {
            $(selectedChar).addClass("char-selected");
...
$(document).on('click', '#create', function(e){
    e.preventDefault();
    $.post('http://rl-multicharacter/createNewCharacter', JSON.stringify({
...
$(document).on('click', '#accept-delete', function(e){
    $.post('http://rl-multicharacter/removeCharacter', JSON.stringify({
...
$(document).on('click', '#play', function(e) {
    ...
    if (selectedChar !== null) {
        if (charData !== "") {
            $.post('http://rl-multicharacter/selectCharacter', JSON.stringify({
