...
local Keys = {
    [108] = "Num 4" ,[110] = "Num 5", [107] = "Num 6", [117] = "Num 7", [111] = "Num 8" , [118] = "Num 9" 
}
...
local function DrawMenu()
    ...
    if WarMenu.Button("Clear Keybinds", "", {r = 135, g = 206, b = 250, a = 150}) then TriggerServerEvent("emotes:clearkeybinds") end
    local key = ""
    for i,v in ipairs(currentKeys) do
...
function addKey(anim,keys)
	...
	TriggerServerEvent("emotes:setkeybinds",currentKeys)
end
...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        if WarMenu.IsMenuOpened("emotes") then
            ...
            DrawText3Ds(plyCoords["x"],plyCoords["y"],plyCoords["z"]+0.1,"~g~Num 4-Num 9~s~ Saves Emote")
            DrawText3Ds(plyCoords["x"],plyCoords["y"],plyCoords["z"],"~g~Shift~s~ with Num keys saves also. (F8 Doesnt Save)")
            ...
            for i,v in ipairs(currentKeys) do
                if v.key[2] ~= nil then
                    if IsDisabledControlJustPressed(0, v.key[1]) then
                       local curButton = WarMenu.GetCurrentButton()
                        if curButton then
