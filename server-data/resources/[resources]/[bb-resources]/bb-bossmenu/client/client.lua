...
RegisterNUICallback('openStash', function(data)
    ...
    TriggerServerEvent("inventory:server:OpenInventory", "stash", "boss_" .. RLCore.Functions.GetPlayerData().job.name, {
        maxweight = 4000000,
        slots = 500,
    })

    TriggerEvent("inventory:client:SetCurrentStash", "boss_" .. RLCore.Functions.GetPlayerData().job.name)
end)
