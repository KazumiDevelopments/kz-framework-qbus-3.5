...
local function LoadPhone()
    Wait(100)
    RLCore.Functions.TriggerCallback('qb-phone:server:GetPhoneData', function(pData)
...
RegisterCommand('phone', function()
    ...
    if not PhoneData.isOpen and LocalPlayer.state.isLoggedIn then
        if not PlayerData.metadata['ishandcuffed'] and not PlayerData.metadata['inlaststand'] and not PlayerData.metadata['isdead'] and not IsPauseMenuActive() then
            ...
            RLCore.Functions.Notify("Action not available at the moment..", "error")
        end
    end
end)
...
RegisterNUICallback('GetProfilePicture', function(data, cb)
    ...
    RLCore.Functions.TriggerCallback('qb-phone:server:GetPicture', function(picture)
        cb(picture)
    end, number)
end)
...
RegisterNUICallback('PayInvoice', function(data, cb)
    ...
    RLCore.Functions.TriggerCallback('qb-phone:server:PayInvoice', function(CanPay, Invoices)
...
RegisterNUICallback('DeclineInvoice', function(data, cb)
    ...
    RLCore.Functions.TriggerCallback('qb-phone:server:DeclineInvoice', function(CanPay, Invoices)
...
RegisterNUICallback('FetchSearchResults', function(data, cb)
    RLCore.Functions.TriggerCallback('qb-phone:server:FetchResult', function(result)
        cb(result)
    end, data.input)
end)
...
if findVehFromPlateAndLocate(veh.plate) then
    RLCore.Functions.Notify("Your vehicle has been marked", "success")
else
    RLCore.Functions.Notify("This vehicle cannot be located", "error")
end
end)
...
RegisterNUICallback('GetCryptoData', function(data, cb)
    RLCore.Functions.TriggerCallback('qb-crypto:server:GetCryptoData', function(CryptoData)
        cb(CryptoData)
    end, data.crypto)
end)

RegisterNUICallback('BuyCrypto', function(data, cb)
    RLCore.Functions.TriggerCallback('qb-crypto:server:BuyCrypto', function(CryptoData)
        cb(CryptoData)
    end, data)
end)

RegisterNUICallback('SellCrypto', function(data, cb)
    RLCore.Functions.TriggerCallback('qb-crypto:server:SellCrypto', function(CryptoData)
        cb(CryptoData)
    end, data)
end)

RegisterNUICallback('TransferCrypto', function(data, cb)
    RLCore.Functions.TriggerCallback('qb-crypto:server:TransferCrypto', function(CryptoData)
        cb(CryptoData)
    end, data)
end)
...
RegisterNUICallback('SetAlertWaypoint', function(data)
    ...
    RLCore.Functions.Notify('GPS set: '..data.alert.title)
...
RegisterNUICallback('FetchVehicleResults', function(data, cb)
    RLCore.Functions.TriggerCallback('qb-phone:server:GetVehicleSearchResults', function(result)
        if result ~= nil then
...
RegisterNUICallback('FetchVehicleScan', function(data, cb)
    ...
    RLCore.Functions.TriggerCallback('qb-phone:server:ScanPlate', function(result)
	    ...
	    if RLCore.Shared.Vehicles[vehname] ~= nil then
                result.label = RLCore.Shared.Vehicles[vehname]['name']
            else
...
RegisterNUICallback('RaceDistanceCheck', function(data, cb)
    RLCore.Functions.TriggerCallback('rl-lapraces:server:GetRacingData', function(RaceData)
        ...
        if dist <= 115.0 then
            ...
            RLCore.Functions.Notify('You\'re too far away from the race. GPS set.', 'error', 5000)
...
RegisterNUICallback('GetPlayerHouses', function(data, cb)
    RLCore.Functions.TriggerCallback('qb-phone:server:GetPlayerHouses', function(Houses)
        cb(Houses)
    end)
end)

RegisterNUICallback('GetPlayerKeys', function(data, cb)
    RLCore.Functions.TriggerCallback('qb-phone:server:GetHouseKeys', function(Keys)
        cb(Keys)
    end)
end)
...
RegisterNUICallback('SetHouseLocation', function(data, cb)
    ...
    RLCore.Functions.Notify("GPS set to " .. data.HouseData.HouseData.adress .. "!", "success")
end)
...
RegisterNUICallback('TransferCid', function(data, cb)
    ...
    RLCore.Functions.TriggerCallback('qb-phone:server:TransferCid', function(CanTransfer)
        cb(CanTransfer)
    end, TransferedCid, data.HouseData)
end)

RegisterNUICallback('FetchPlayerHouses', function(data, cb)
    RLCore.Functions.TriggerCallback('qb-phone:server:MeosGetPlayerHouses', function(result)
        cb(result)
    end, data.input)
end)
...
RegisterNUICallback('SetGPSLocation', function(data, cb)
    ...
    RLCore.Functions.Notify('GPS set!', 'success')
end)
...
RegisterNUICallback('GetCurrentLawyers', function(data, cb)
    RLCore.Functions.TriggerCallback('qb-phone:server:GetCurrentLawyers', function(lawyers)
        cb(lawyers)
    end)
end)
...
RegisterNUICallback('CanTransferMoney', function(data, cb)
    ...
    if (PlayerData.money.bank - amount) >= 0 then
        RLCore.Functions.TriggerCallback('qb-phone:server:CanTransferMoney', function(Transferd)
            if Transferd then
...
RegisterNUICallback('GetWhatsappChats', function(data, cb)
    RLCore.Functions.TriggerCallback('qb-phone:server:GetContactPictures', function(Chats)
        cb(Chats)
    end, PhoneData.Chats)
end)

RegisterNUICallback('CallContact', function(data, cb)
    RLCore.Functions.TriggerCallback('qb-phone:server:GetCallState', function(CanCall, IsOnline, contactData)
        local status = {
            CanCall = CanCall,
            IsOnline = IsOnline,
...
RegisterNUICallback('SendMessage', function(data, cb)
    ...
    RLCore.Functions.TriggerCallback('qb-phone:server:GetContactPicture', function(Chat)
        SendNUIMessage({
            action = "UpdateChat",
            chatData = Chat,
...
RegisterNUICallback("TakePhoto", function(data,cb)
    ...
    while takePhoto do
        ...
        elseif IsControlJustPressed(1, 176) then
            RLCore.Functions.TriggerCallback("qb-phone:server:GetWebhook",function(hook)
                RLCore.Functions.Notify('Touching up photo...', 'primary')
                exports['screenshot-basic']:requestScreenshotUpload(tostring(hook), "files[]", function(data)
                    ...
                    RLCore.Functions.Notify('Photo ssaved!', 'success')
                    OpenPhone()
                end)
            end)
...
RegisterNUICallback('phone-silent-button', function(data,cb)
    if CallVolume == tonumber("0.2") then
        CallVolume = 0
        RLCore.Functions.Notify("Silent Mode On", "success")
        cb(true)
    else
        CallVolume = 0.2
        RLCore.Functions.Notify("Silent Mode Off", "error")
        cb(false)
    end
end)
...
RegisterNetEvent('qb-phone:client:GiveContactDetails', function()
    ...
    if player ~= -1 and distance < 2.5 then
        ...
        RLCore.Functions.Notify("No one nearby!", "error")
    end
end)
...
RegisterNetEvent('qb-pings:client:SendLocation', function(PingData, SenderData)
    RLCore.Functions.Notify('Their location has been blipped on your map', 'success')
...
RegisterNUICallback('CheckHasBetTable', function(data, cb)
    RLCore.Functions.TriggerCallback('qb-phone:server:CheckHasBetTable', function(HasTable)
        cb(HasTable)
    end)
end)
...
RegisterNUICallback('CheckHasBetStatus', function(data, cb)
    RLCore.Functions.TriggerCallback('qb-phone:server:CheckHasBetStatus', function(HasStatus)
        cb(HasStatus)
    end)
end)
...
RegisterNUICallback('GetGroupsApp', function(data, cb)
    RLCore.Functions.TriggerCallback('qb-phone:server:GetGroupsApp', function(HasGroups)
        cb(HasGroups)
    end)
end)
...
RegisterNUICallback('employment_CheckPlayerNames', function(data, cb)
    RLCore.Functions.TriggerCallback('qb-phone:server:employment_CheckPlayerNames', function(HasName)
        cb(HasName)
    end, data.id)
end)
...
RegisterNUICallback('GetHasBills_debt', function(data, cb)
    RLCore.Functions.TriggerCallback('qb-phone:server:GetHasBills_debt', function(Has)
        cb(Has)
    end)
end)
...
RegisterNUICallback('GetNote_for_Documents_app', function(data, cb)
    RLCore.Functions.TriggerCallback('qb-phone:server:GetNote_for_Documents_app', function(Has)
        cb(Has)
    end)
end)
...
local function GetGroupCSNs(Data)
    local gData = {}
    RLCore.Functions.TriggerCallback('qb-phone:server:GetGroupCSNs', function(HasGroup)
        gData = HasGroup
    end, Data)
    Wait(100)
    return gData
