...
RegisterNetEvent('rl-scenes:client:UpdateAllScenes', function(_scenes)
    scenes = _scenes
end)
...
function GetScenes()
    RLCore.Functions.TriggerCallback('rl-scenes:server:GetScenes', function(_scenes)
        scenes = _scenes
    end)
end
        ...
        CreateThread(function()
            while creationLaser do
                ...
                if IsControlJustReleased(0, 38) then
                    creationLaser = false
                    if hit then
                        TriggerServerEvent('rl-scenes:server:CreateScene', data)
                    else
                        RLCore.Functions.Notify(Lang:t("notify.laser_error"), "error")
                    end
...
function DeleteScene(coords)
    ...
    if closestScene then
        RLCore.Functions.Notify(Lang:t("notify.scene_delete"), "success")
        TriggerServerEvent('rl-scenes:server:DeleteScene', closestScene)
    else
        RLCore.Functions.Notify(Lang:t("notify.scene_error"), "error")
    end
end
