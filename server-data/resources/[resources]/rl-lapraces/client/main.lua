...
RegisterNetEvent('rl-lapraces:client:StartRaceEditor')
AddEventHandler('rl-lapraces:client:StartRaceEditor', function(RaceName)
    if not RaceData.InCreator then
        ...
        RLCore.Functions.Notify('You are making a race.')
    end 
end)
        ...
        while RaceData.InCreator do
            ...
            if PlayerVeh ~= 0 then
                ...
                if IsControlJustPressed(0, Keys["8"]) or IsDisabledControlJustPressed(0, Keys["8"]) then
                    if CreatorData.Checkpoints ~= nil and next(CreatorData.Checkpoints) ~= nil then
                        ...
                        RLCore.Functions.Notify('You have not yet placed checkpoints.', 'error')
                    end
                end
                ...
                if IsControlJustPressed(0, Keys["K"]) or IsDisabledControlJustPressed(0, Keys["K"]) then
                    if CreatorData.Checkpoints ~= nil and #CreatorData.Checkpoints >= 2 then
                        ...
                        RLCore.Functions.Notify('You must have at least 10 checkpoints.', 'error')
                    end
                end
                ...
                if IsControlJustPressed(0, Keys["]"]) or IsDisabledControlJustPressed(0, Keys["]"]) then
                    if CreatorData.TireDistance + 1.0 ~= 16.0 then
                        ...
                        RLCore.Functions.Notify('You cannot go higher than 15 meters.')
                    end
                end
                ...
                if IsControlJustPressed(0, Keys["["]) or IsDisabledControlJustPressed(0, Keys["["]) then
                    if CreatorData.TireDistance - 1.0 ~= 1.0 then
                        ...
                        RLCore.Functions.Notify('You cannot go lower than 2 meters.')
                    end
                end
            else
            ...
            if IsControlJustPressed(0, Keys["9"]) or IsDisabledControlJustPressed(0, Keys["9"]) then
                if not CreatorData.ConfirmDelete then
                    ...
                    RLCore.Functions.Notify('Press [9] again to confirm', 'error', 5000)
                else
                    ...
                    RLCore.Functions.Notify('Race editor canceled.', 'error')
...
function SaveRace()
    ...
    TriggerServerEvent('rl-lapraces:server:SaveRace', CreatorData)

    RLCore.Functions.Notify('Race: '..CreatorData.RaceName..' is stored!', 'success')
...
function DeleteCheckpoint()
    ...
    if RaceData.ClosestCheckpoint ~= 0 then
        if CreatorData.Checkpoints[RaceData.ClosestCheckpoint] ~= nil then
            ...
            RLCore.Functions.Notify('You cant go too fast.', 'error')
        end
    else
        RLCore.Functions.Notify('You cant go too fast.', 'error')
    end
...
RegisterNetEvent('rl-lapraces:client:UpdateRaceRacerData')
AddEventHandler('rl-lapraces:client:UpdateRaceRacerData', function(RaceId, RaceData)
...
RegisterNetEvent('rl-lapraces:client:JoinRace')
AddEventHandler('rl-lapraces:client:JoinRace', function(Data, Laps)
    if not RaceData.InRace then
        RaceData.InRace = true
        SetupRace(Data, Laps)
        TriggerServerEvent('rl-lapraces:server:UpdateRaceState', CurrentRaceData.RaceId, false, true)
    else
        RLCore.Functions.Notify('You joined the race.')
    end
end)

RegisterNetEvent('rl-lapraces:client:LeaveRace')
AddEventHandler('rl-lapraces:client:LeaveRace', function(data)
    RLCore.Functions.Notify('You left the race.', "error")
    for k, v in pairs(CurrentRaceData.Checkpoints) do
...
RegisterNetEvent('rl-lapraces:client:RaceCountdown')
AddEventHandler('rl-lapraces:client:RaceCountdown', function()
    TriggerServerEvent('rl-lapraces:server:UpdateRaceState', CurrentRaceData.RaceId, true, false)
    if CurrentRaceData.RaceId ~= nil then
        while Countdown ~= 0 do
            if CurrentRaceData.RaceName ~= nil then
                if Countdown == 10 then
                    RLCore.Functions.Notify('The race will start in 10 seconds.', "error", 2500)
                    PlaySound(-1, "slow", "SHORT_PLAYER_SWITCH_SOUND_SET", 0, 0, 1)
                elseif Countdown <= 5 then
                    RLCore.Functions.Notify(Countdown, 'error', 500)
                    PlaySound(-1, "slow", "SHORT_PLAYER_SWITCH_SOUND_SET", 0, 0, 1)
                end
        ...
        RLCore.Functions.Notify('You are not in a race.', 'error')
    end
end)
                ...
                if CheckpointDistance < MaxDistance then
                    if CurrentRaceData.TotalLaps == 0 then
                        if CurrentRaceData.CurrentCheckpoint + 1 < #CurrentRaceData.Checkpoints then
                            ...
                            SetNewWaypoint(CurrentRaceData.Checkpoints[CurrentRaceData.CurrentCheckpoint + 1].coords.x, CurrentRaceData.Checkpoints[CurrentRaceData.CurrentCheckpoint + 1].coords.y)
                            TriggerServerEvent('rl-lapraces:server:UpdateRacerData', CurrentRaceData.RaceId, CurrentRaceData.CurrentCheckpoint, CurrentRaceData.Lap, false)
                            DoPilePfx()
                            ...
                            PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
                            CurrentRaceData.CurrentCheckpoint = CurrentRaceData.CurrentCheckpoint + 1
                            TriggerServerEvent('rl-lapraces:server:UpdateRacerData', CurrentRaceData.RaceId, CurrentRaceData.CurrentCheckpoint, CurrentRaceData.Lap, true)
                            FinishRace()
                        end
                    else
                        if CurrentRaceData.CurrentCheckpoint + 1 > #CurrentRaceData.Checkpoints then
...
function FinishRace()
    TriggerServerEvent('rl-lapraces:server:FinishPlayer', CurrentRaceData, CurrentRaceData.TotalTime, CurrentRaceData.TotalLaps, CurrentRaceData.BestLap)
    if CurrentRaceData.BestLap ~= 0 then
        RLCore.Functions.Notify('Race finished in '..SecondsToClock(CurrentRaceData.TotalTime)..', with the best lap: '..SecondsToClock(CurrentRaceData.BestLap))
    else
        RLCore.Functions.Notify('Race finished in '..SecondsToClock(CurrentRaceData.TotalTime))
    end
    for k, v in pairs(CurrentRaceData.Checkpoints) do
        if CurrentRaceData.Checkpoints[k].blip ~= nil then
            RemoveBlip(CurrentRaceData.Checkpoints[k].blip)
...
RegisterNetEvent('rl-lapraces:client:PlayerFinishs')
AddEventHandler('rl-lapraces:client:PlayerFinishs', function(RaceId, Place, FinisherData)
    if CurrentRaceData.RaceId ~= nil then
        if CurrentRaceData.RaceId == RaceId then
            RLCore.Functions.Notify(FinisherData.PlayerData.charinfo.firstname..' is finished on the spot: '..Place, 'error', 3500)
        end
    end
end)
...
RegisterNetEvent('rl-lapraces:client:WaitingDistanceCheck')
AddEventHandler('rl-lapraces:client:WaitingDistanceCheck', function()
    Wait(1000)
    Citizen.CreateThread(function()
        while true do
                ...
                if CurrentRaceData.Checkpoints[1] ~= nil then
                    ...
                    if dist > 115.0 then
                        if ToFarCountdown ~= 0 then
                            ...
                            RLCore.Functions.Notify('Go back to the start or you will be disqualified in: '..ToFarCountdown..'s', 'error', 500)
                        else
                            TriggerServerEvent('rl-lapraces:server:LeaveRace', CurrentRaceData)
                            ToFarCountdown = 10
                            break
                        end
                        Citizen.Wait(1000)
